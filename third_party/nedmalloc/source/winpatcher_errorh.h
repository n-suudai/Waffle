...
#define MKSTATUS(ret, codev)                                                   \
    (ret.code = codev, ret.sourcefile = __FILE__, ret.sourcelineno = __LINE__, \
     ret)
#define MKSTATUSWIN(ret) MakeStatusFromWindowsError(&ret, __FILE__, __LINE__)

static Status MakeStatusFromWindowsError(Status *ret, const char *file,
                                         int lineno)
{
    ...
    FormatMessage(FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS,
                  NULL, -ret->code, 0, ret->msg,
                  (sizeof(ret->msg) / sizeof(TCHAR)) - 1, NULL);
    ret->msg[64] = 0;
    return *ret;
}

static void MakeReportFromStatus(TCHAR *buffer, size_t bufflen,
                                 const Status *ret)
{
    TCHAR *p;
    const char *s;
    if (ret->code >= 0)
        ...
        _stprintf_s(buffer, bufflen, __T("Error code %d (%s) at "), ret->code,
                    ret->msg);
