...
/*#define USERMODEPAGEALLOCATOR_FREEPAGECACHESIZE(usedpages, freepages)
 * (usedpages)*/
#define USERMODEPAGEALLOCATOR_FREEPAGECACHESIZE(usedpages, freepages) \
    ((size_t) - 1)
...
large amounts of memory getting repeatedly freed and reallocated. It gets
ignored
...
/*#define USERMODEPAGEALLOCATOR_FREEPAGECACHEPRELOAD(systemmemorypressure)
 * ((512*1024*1024/4096))*/
...
/* This turns on the storage of free page metadata directly in the page tables
which
will halve the page table memory requirements. As on x86/x64 page frames start
from
one going upwards sequentially and will never use the top bit, this ought to
always
be safe. */
...
much buffering and caching is disabled as possible in order to best test the
code. */
#ifdef DEBUG
...
/*
#if (defined(__GNUC__) && (defined(__i386__) || defined(__x86_64__)))
#define PREFETCHCACHELINE(addr, rw, locality) __builtin_prefetch((const void
*)(addr), (rw), (locality))
#define STRUCTUREALIGNMENT(alignment)         __attribute__
((aligned(alignment)))
#elif (defined(_MSC_VER) && (defined(_M_IX86) || defined(_M_X64)))
#define PREFETCHCACHELINE(addr, rw, locality) _mm_prefetch((const char *)(addr),
(locality))
...
extern void *userpage_realloc(void *mem, size_t oldsize, size_t newsize,
                              int flags, unsigned flags2);
...
#define REGION_GENERATE(proto, treetype, nodetype, link, cmpfunct) \
    NEDTRIE_GENERATE(proto, treetype, nodetype, link, cmpfunct,    \
                     NEDTRIE_NOBBLEZEROS(treetype))
#define REGION_INSERT(treetype, treevar, node) \
    NEDTRIE_INSERT(treetype, treevar, node)
#define REGION_REMOVE(treetype, treevar, node) \
    NEDTRIE_REMOVE(treetype, treevar, node)
#define REGION_FIND(treetype, treevar, node) \
    NEDTRIE_FIND(treetype, treevar, node)
#define REGION_EXACTFIND(treetype, treevar, node) \
    NEDTRIE_EXACTFIND(treetype, treevar, node)
#define REGION_CFIND(treetype, treevar, node, rounds) \
    NEDTRIE_CFIND(treetype, treevar, node, rounds)
...
#define REGION_NEXT(treetype, treevar, node) \
    NEDTRIE_NEXT(treetype, treevar, node)
#define REGION_PREV(treetype, treevar, node) \
    NEDTRIE_PREV(treetype, treevar, node)
#define REGION_FOREACH(var, treetype, treevar) \
    NEDTRIE_FOREACH(var, treetype, treevar)
#define REGION_HASNODEHEADER(treevar, node, link) \
    NEDTRIE_HASNODEHEADER(treevar, node, link)
...
{
    regionA_tree_t
    regiontreeA; /* The list of allocated regions, keyed by start addr */
...
{
    OSAddressSpaceReservationData asrd = {0};
    return asrd;
}
...
static size_t OSObtainMemoryPages(PageFrameType *buffer, size_t number,
                                  OSAddressSpaceReservationData *data)
{
    return 0;
}

/* This function returns previously obtained physical memory pages. */
static size_t OSReleaseMemoryPages(PageFrameType *buffer, size_t number,
                                   OSAddressSpaceReservationData *data)
{
    return 0;
}
...
*/
static size_t OSRemapMemoryPagesOntoAddr(void *addr, size_t entries,
                                         PageFrameType *pageframes,
                                         OSAddressSpaceReservationData *data)
{
    return 0;
}
...
*/
static size_t OSRemapMemoryPagesOntoAddrs(void **addrs, size_t entries,
                                          PageFrameType *pageframes,
                                          OSAddressSpaceReservationData *data)
{
    return 0;
...
static int OSDeterminePhysicalPageSupport(void)
{
    ...
    {/* Quick test */
        ...
        SYSTEM_INFO si = {0};
        {
                                 ...
                                 &token))
            {
                                         ...
                                         &privs.Privileges[0].Luid))
                {
                    ...
                    if (!AdjustTokenPrivileges(token, FALSE, &privs, 0, NULL,
                                               NULL) ||
                        GetLastError() != S_OK)
                    {
                    }
        ...
        if (AllocateUserPhysicalPages((HANDLE)(size_t) - 1, (PULONG_PTR) & no,
                                      (PULONG_PTR) & pageframe))
        {
            FreeUserPhysicalPages((HANDLE)(size_t) - 1, (PULONG_PTR) & no,
                                  (PULONG_PTR) & pageframe);
            PhysicalPageSupport = HAVEPHYSICALPAGESUPPORT;
            CreateEvent(NULL, FALSE, FALSE,
                        __T("UserModePageAllocatorEnabled"));
        }
        else
        {
            PhysicalPageSupport = NOPHYSICALPAGESUPPORT;
            /*fprintf(stderr, "User Mode Page Allocator: Failed to allocate
             * physical memory pages (does the user running this process have
             * the right to lock pages in memory?). User Mode Page Allocator
             * will not be used.\n");*/
            OutputDebugStringA(
              "User Mode Page Allocator: Failed to allocate physical memory "
              "pages (does the user running this process have the right to "
              "lock pages in memory?). User Mode Page Allocator will not be "
              "used.\n");
            CreateEvent(NULL, FALSE, FALSE,
                        __T("UserModePageAllocatorDisabled"));
        }
        GetSystemInfo(&si);
#ifdef ENABLE_PHYSICALPAGEEMULATION
        if (si.dwAllocationGranularity != PAGE_SIZE)
        {
            ...
            fprintf(stderr,
                    "User Mode Page Allocator: Allocation granularity is %u "
                    "not %u. Please recompile with corrected PAGE_SIZE\n",
                    si.dwAllocationGranularity, PAGE_SIZE);
            PhysicalPageSupport = DISABLEEVERYTHING;
        }
        if (si.dwAllocationGranularity / si.dwPageSize !=
            sizeof(PageFrameType) / sizeof(ULONG_PTR))
        {
            assert(si.dwAllocationGranularity / si.dwPageSize ==
                   sizeof(PageFrameType) / sizeof(ULONG_PTR));
            fprintf(stderr,
                    "User Mode Page Allocator: Pages per PageFrameType is %u "
                    "not %u. Please recompile with corrected PageFrameType "
                    "definition\n",
        ...
        if (si.dwPageSize != PAGE_SIZE)
        {
            ...
            fprintf(stderr,
                    "User Mode Page Allocator: Page size is %u not %u. Please "
                    "recompile with corrected PAGE_SIZE\n",
...
{
    if (!PhysicalPageSupport)
        OSDeterminePhysicalPageSupport();
...
static OSAddressSpaceReservationData OSReserveAddrSpace(size_t space)
{
    ...
    if (!PhysicalPageSupport)
        OSDeterminePhysicalPageSupport();
    if (DISABLEEVERYTHING == PhysicalPageSupport)
        return ret;
    if (HAVEPHYSICALPAGESUPPORT == PhysicalPageSupport)
    {
        ret.addr =
          VirtualAlloc(NULL, space, MEM_RESERVE | MEM_PHYSICAL, PAGE_READWRITE);
    }
#ifdef ENABLE_PHYSICALPAGEEMULATION
    if (!ret.addr)
    {
        ...
        {/* This is breathtakingly inefficient, but win32 leaves us no choice :(
            ...
            while ((ret.addr =
                      VirtualAlloc(NULL, space, MEM_RESERVE, PAGE_READWRITE)))
            {
                void *RESTRICT seg;
                VirtualFree(ret.addr, 0, MEM_RELEASE);
                for (seg = ret.addr; seg < (void *)((size_t)ret.addr + space);
                     seg = (void *)((size_t)seg + Win32granularity))
                {
                    if (!VirtualAlloc(seg, Win32granularity, MEM_RESERVE,
                                      PAGE_READWRITE))
                        break;
                }
...
static int OSReleaseAddrSpace(OSAddressSpaceReservationData *RESTRICT data,
                              size_t space)
{
    if (!data->data[0])
    ...
    else
    {
        ...
        for (seg = data->addr; seg < (void *)((size_t)data->addr + space);
             seg = (void *)((size_t)seg + Win32granularity))
            VirtualFree(seg, 0, MEM_RELEASE);
        return 1;
    }
...
static size_t OSObtainMemoryPages(PageFrameType *RESTRICT buffer, size_t number,
                                  OSAddressSpaceReservationData *RESTRICT data)
{
    if (!data->data[0])
    {
#ifdef ENABLE_PHYSICALPAGEEMULATION
        ...
        if (!AllocateUserPhysicalPages(
               (HANDLE)(size_t) - 1, (PULONG_PTR) & number, (PULONG_PTR)buffer))
        {
            if (number)
                FreeUserPhysicalPages((HANDLE)(size_t) - 1,
                                      (PULONG_PTR) & number,
                                      (PULONG_PTR)buffer);
            return 0;
        }
#else
        {
            ...
            {
                size_t no = 1;
                if (!AllocateUserPhysicalPages((HANDLE)(size_t) - 1,
                                               (PULONG_PTR) & no,
                                               (PULONG_PTR)bptr))
                {
    ...
    else
    {
        ...
        for (n = 0; n < number * (sizeof(PageFrameType) / sizeof(ULONG_PTR));
             n++)
...
static size_t OSReleaseMemoryPages(PageFrameType *RESTRICT buffer,
                                   size_t number,
                                   OSAddressSpaceReservationData *RESTRICT data)
{
    if (!data->data[0])
...
#endif
#if 1
        if (!FreeUserPhysicalPages((HANDLE)(size_t) - 1, (PULONG_PTR) & number,
                                   (PULONG_PTR)buffer))
            return 0;
#else
        {
            ...
            {
                size_t no = 1;
                if (!FreeUserPhysicalPages((HANDLE)(size_t) - 1,
                                           (PULONG_PTR) & no, (PULONG_PTR)bptr))
                {
                    assert(0);
                }
            }
...
static __declspec(noinline) size_t OSRemapMemoryPagesOntoAddr(
  void *addr, size_t entries, PageFrameType *RESTRICT pageframes,
  OSAddressSpaceReservationData *RESTRICT data)
{
    if (!data->data[0])
    {
...
#else
        {
            size_t n;
            PageFrameType *RESTRICT bptr;
            for (n = 0, bptr = pageframes; n < entries;
                 n++, bptr++, addr = (void *)((size_t)addr + PAGE_SIZE))
            {
    ...
    else
    {
        ...
        for (n = 0; n < entries;
             n++, addr = (void *)((size_t)addr + PAGE_SIZE), pageframes++)
        {
            if (*pageframe)
            {
                ...
                if (!MapViewOfFileEx((HANDLE)data.data[0], FILE_MAP_ALL_ACCESS,
                                     ...
                                     0,
#endif
                                     (DWORD)(filemappingoffset & (DWORD) - 1),
                                     PAGE_SIZE, addr))
                    ret = 0;
            }
            else
            {
                if (!UnmapViewOfFile(addr))
                    ret = 0;
                /* Rereserve */
                if (!VirtualAlloc(addr, PAGE_SIZE, MEM_RESERVE, PAGE_READWRITE))
                    ret = 0;
            }
        }
        return ret;
...
static __declspec(noinline) size_t OSRemapMemoryPagesOntoAddrs(
  void *RESTRICT *addrs, size_t entries, PageFrameType *RESTRICT pageframes,
  OSAddressSpaceReservationData *RESTRICT data)
{
#ifdef DEBUG
    size_t n;
    ...
    if (!data->data[0])
    {
...
#if 1
        ret =
          MapUserPhysicalPagesScatter(addrs, entries, (PULONG_PTR)pageframes);
        if (!ret)
        {
            assert(ret);
        }
#else
        {
            ...
            void **aptr;
            PageFrameType *RESTRICT bptr;
            for (n = 0, aptr = addrs, bptr = pageframes; n < entries;
                 n++, aptr++, bptr++)
            {
    ...
    else
    {
        ...
        for (n = 0; n < entries; n++, addrs++, pageframes++)
        {
            if (*pageframe)
            {
                size_t filemappingoffset = PAGE_SIZE * ((*pageframe) - 1);
                /* Change reservation for next segment */
                if (!VirtualFree(*addrs, 0, MEM_RELEASE))
                    ret = 0;
                if (!MapViewOfFileEx((HANDLE)data.data[0], FILE_MAP_ALL_ACCESS,
#if defined(_M_IA64) || defined(_M_X64) || defined(WIN64)
                                     (DWORD)(filemappingoffset >> 32),
            ...
            {
                if (!UnmapViewOfFile(*addrs))
                    ret = 0;
                /* Rereserve */
                if (!VirtualAlloc(*addrs, PAGE_SIZE, MEM_RESERVE,
                                  PAGE_READWRITE))
                    ret = 0;
            }
        }
...
#endif
{
    PageFrameType pageframe; /* Actually the pageframe shifted left by one and
                                the bottom bit always set */
...
#define PAGEFRAME(pagemapping)                   \
    (ISPAGEFREE(pagemapping)                     \
       ? ((pagemapping).freepagenode)->pageframe \
       : (PageFrameType)((size_t)((pagemapping).pageframe) >> 1))
#define SETPAGEFRAME(pagemapping, pf) \
    ((pagemapping).pageframe =        \
       !(pf) ? 0 : (PageFrameType)(((size_t)(pf) << 1) | 1))
#define SETPAGEFREE(pagemapping, fpn)                           \
    ((fpn)->pageframe =                                         \
       (PageFrameType)((size_t)((pagemapping).pageframe) >> 1), \
     (pagemapping).freepagenode = (fpn))
...
#define SETPAGEFREE(pagemapping, fpn)              \
    ((fpn)->pageframe = ((pagemapping).pageframe), \
     (pagemapping).freepagenode = (fpn))
#define SETPAGEUSED(pagemapping, pf) \
    (SETPAGEFRAME((pagemapping), (pf)), (pagemapping).freepagenode = 0)
#endif
...
static void ValidateFreePageLists(AddressSpaceReservation_t *RESTRICT addr)
{
    ...
    for (fpn = addr->oldestclean; fpn; fpn = fpn->newer)
    {
        ...
        assert((!fpn->older && addr->oldestclean == fpn) ||
               fpn->older->newer == fpn);
        assert((!fpn->newer && addr->newestclean == fpn) ||
               fpn->newer->older == fpn);
    ...
    for (fpn = addr->oldestdirty; fpn; fpn = fpn->newer)
    {
        ...
        assert(ISPAGEFREE(addr->pagemapping[freepagepfidx]));
        assert(addr->pagemapping[freepagepfidx].freepagenode == fpn);
        assert((!fpn->older && addr->oldestdirty == fpn) ||
               fpn->older->newer == fpn);
        assert((!fpn->newer && addr->newestdirty == fpn) ||
               fpn->newer->older == fpn);
        assert(fpn->age <= addr->opcount);
        assert(fpn->dirty);
        count++;
...
{
    const size_t RESERVEALWAYSLEAVEFREE =
      64 * 1024 * 1024; /* Windows goes seriously screwy if you take away all
                           address space */
    ...
    if (space < (size_t)1 << 30 /* 1Gb */)
    {
        space = (size_t)1 << 30;
        if (8 == sizeof(size_t))
            space <<= 2; /* Go for 4Gb chunks on 64 bit */
    }
    while (space >= RESERVEALWAYSLEAVEFREE &&
           !(addrR = OSReserveAddrSpace(space)).addr)
        space >>= 1;
    if (space < RESERVEALWAYSLEAVEFREE)
        return 0;
    pagemappingsize = sizeof(AddressSpaceReservation_t) +
                      sizeof(PageMapping) * ((space / PAGE_SIZE) - 2);
    pagemappingsize = (pagemappingsize + PAGE_SIZE - 1) & ~(PAGE_SIZE - 1);
    pagemappingsize /= PAGE_SIZE;
    /* We now need pagemappingsize number of pages in order to store the mapping
    tables, but
    because this could be as much as 4Mb of stuff we'll need to do it in chunks
    to avoid
    breaking the stack. */
    for (n = 0; n < pagemappingsize; n += pagesallocated)
    {
        ...
        if (torequest > pagemappingsize - n)
            torequest = pagemappingsize - n;
        if (!(pagesallocated =
                OSObtainMemoryPages(pagebuffer, torequest, &addrR)))
            goto badexit;
        if (!OSRemapMemoryPagesOntoAddr(mapaddr, pagesallocated, pagebuffer,
                                        &addrR))
            goto badexit;
        if (!n)
        {/* This is the first run, so install AddressSpaceReservation */
            ...
            addr->front = addr->frontptr =
              (void *)((size_t)addr + pagemappingsize * PAGE_SIZE);
            addr->back = addr->backptr = (void *)((size_t)addr + space);
        }
        /* Add these new pages to the page mappings. Because we are premapping
        in new pages,
        we are guaranteed to have memory already there ready for us. */
        for (torequest = 0; torequest < pagesallocated; torequest++)
            SETPAGEFRAME(addr->pagemapping[n + torequest],
                         pagebuffer[torequest]);
    }
    ValidatePageMappings(addr);
#ifdef DEBUG
    DebugPrint("*** Reserved address space from %p to %p (%luMb)\n", addr,
               addr->back, (unsigned long)((size_t)addr->back - (size_t)addr) /
                             1024 / 1024);
#endif
    return addr;
badexit:
...
static int CheckFreeAddressSpaces(AddressSpaceReservation_t *RESTRICT *RESTRICT
                                    _addr)
{
...
static size_t FillWithFreePages(AddressSpaceReservation_t *RESTRICT addr,
                                RemapMemoryPagesBlock *RESTRICT memtodecommit,
                                RemapMemoryPagesBlock *RESTRICT memtocommit,
                                void *freespaceaddr,
                                PageMapping *RESTRICT start,
                                PageMapping *RESTRICT end, int needclean)
{
    size_t n, pages = end - start;
    PageMapping *RESTRICT pf = start;
    for (n = 0; n < pages && addr->freepages;
         n++, pf++, freespaceaddr = (void *)((size_t)freespaceaddr + PAGE_SIZE))
    {
        FreePageNode *RESTRICT fpn = 0, *RESTRICT *RESTRICT fpnaddr = 0,
                               *RESTRICT *RESTRICT fpnnaddr = 0;
        ...
        *fpnaddr = fpn->newer;
        ...
        if (REMAPMEMORYPAGESBLOCKSIZE == memtocommit->idx)
        {
            if (memtodecommit->idx)
                OSRemapMemoryPagesOntoAddrs(memtodecommit->addrs,
                                            memtodecommit->idx, NULL,
                                            &addr->OSreservedata);
            OSRemapMemoryPagesOntoAddrs(memtocommit->addrs, memtocommit->idx,
                                        memtocommit->pageframes,
                                        &addr->OSreservedata);
...
#ifdef DEBUG
    if (pages - n > 0)
        DebugPrint("Requesting %lu new pages from kernel\n",
                   (unsigned long)pages - n);
#endif
    while (pages - n > 0)
    {
        size_t newpagesnow = pages - n, newpagesobtained, m;
        if (newpagesnow > REMAPMEMORYPAGESBLOCKSIZE - memtocommit->idx)
            newpagesnow = REMAPMEMORYPAGESBLOCKSIZE - memtocommit->idx;
        newpagesobtained =
          OSObtainMemoryPages(&memtocommit->pageframes[memtocommit->idx],
                              newpagesnow, &addr->OSreservedata);
        if (newpagesnow != newpagesobtained)
        {
            if (newpagesobtained)
                OSReleaseMemoryPages(&memtocommit->pageframes[memtocommit->idx],
                                     newpagesobtained, &addr->OSreservedata);
            return n;
        }
        for (m = memtocommit->idx; m < memtocommit->idx + newpagesobtained; m++,
            pf++, freespaceaddr = (void *)((size_t)freespaceaddr + PAGE_SIZE))
        {
...
static int DetachFreePage(AddressSpaceReservation_t *RESTRICT addr,
                          FreePageNode *RESTRICT fpn)
{
    int wipeall = 0;
    FreePageNode *RESTRICT *RESTRICT prevnextaddr = 0,
                                     *RESTRICT *RESTRICT nextprevaddr = 0;
    if (fpn->older)
    {
...
static void *AllocatePages(void *mem, size_t size, unsigned flags)
{
    ...
    if (!addressspacereservation &&
        !(addressspacereservation = ReserveSpace(0)))
    {
        DebugPrint(
          "User Mode Page Allocator: Failed to allocate initial address "
          "space\n");
        abort();
    }
    for (addr = addressspacereservation; addr; addr = addr->next)
    {
        ...
        if ((mem &&
             ((mem >= addr->front && mem < addr->frontptr && !(fromback = 0)) ||
              (mem >= addr->backptr && mem < addr->back && (fromback = 1)))) ||
            (!mem && ((size_t)addr->backptr - (size_t)addr->frontptr >= size)))
        {
            ...
            void *ret = mem ? mem : ((fromback)
                                       ? (void *)((size_t)addr->backptr - size)
                                       : addr->frontptr),
                 *retptr;
            PageMapping *RESTRICT
            pagemappingsbase =
              addr->pagemapping + ((size_t)ret - (size_t)addr) / PAGE_SIZE,
            *RESTRICT pagemappings;
            ...
            if (!mem)
            {
                ...
                DebugPrint("VA %s goes to %p within %p-%p\n",
                           fromback ? "backptr" : "frontptr",
                           fromback ? addr->backptr : addr->frontptr,
                           addr->front, addr->back);
#endif
            }
            if (!(flags & USERPAGE_NOCOMMIT))
            {/* We leave memory still held by the application mapped at the
             addresses it was mapped at
             when freed and only nobble these when we need new pages. Hence
             between addresses ret and
             ret+size there may be a patchwork of already allocated regions, so
             what we do is to firstly
             delink any already mapped pages from the free page list and then to
             batch the filling in of
             the blank spots sixteen at a time. */
                pagemappings = pagemappingsbase;
                retptr = ret;
                for (n = 0; n < sizeinpages; n++, pagemappings++,
                    retptr = (void *)((size_t)retptr + PAGE_SIZE))
                {
                ...
                if (needtofillwithfree)
                {
                        ...
                        retptr = (void *)((size_t)retptr + PAGE_SIZE))
                    {
                        if (!pagemappings->pageframe)
                        {
                            ...
                            PageMapping *RESTRICT emptyframestart =
                              pagemappings;
                            size_t filled;
                            for (; n < sizeinpages && !pagemappings->pageframe;
                                 n++, pagemappings++,
                                 retptr = (void *)((size_t)retptr + PAGE_SIZE))
                                ;
                            if ((size_t)(pagemappings - emptyframestart) !=
                                (filled = FillWithFreePages(
                                   addr, &memtodecommit, &memtocommit,
                                   emptyaddrstart, emptyframestart,
                                   pagemappings, fromback)))
                    ...
                    if (memtocommit.idx)
                    {
                        if (memtodecommit.idx)
                            OSRemapMemoryPagesOntoAddrs(memtodecommit.addrs,
                                                        memtodecommit.idx, NULL,
                                                        &addr->OSreservedata);
                        OSRemapMemoryPagesOntoAddrs(
                          memtocommit.addrs, memtocommit.idx,
                          memtocommit.pageframes, &addr->OSreservedata);
                        memtodecommit.idx = memtocommit.idx = 0;
                        ValidatePageMappings(addr);
                    }
                }
        ...
        {
            addr->next =
              ReserveSpace((size_t)1 << (nedtriebitscanr(size - 1) + 1));
        }
    }
    return 0;
}
static AddressSpaceReservation_t *RESTRICT
AddressSpaceFromMem(int *RESTRICT fromback, void *mem)
{
    AddressSpaceReservation_t *RESTRICT addr;
    for (addr = addressspacereservation; addr; addr = addr->next)
    {
...
static int IsPageFreeOrEmpty(AddressSpaceReservation_t *RESTRICT addr,
                             void *mem)
{
    PageMapping *RESTRICT pagemappings =
      addr->pagemapping + ((size_t)mem - (size_t)addr) / PAGE_SIZE;
    return !pagemappings->pageframe ? 2 : (ISPAGEFREE(*pagemappings) ? 1 : 0);
}
static size_t TrimFreePagesFromAddr(AddressSpaceReservation_t *RESTRICT addr,
                                    size_t pagestofree, size_t pagesmustfree)
{
    size_t pagesfreed = 0;
    int done = 0;
    FreePageNode *RESTRICT fpn;
    ...
    for (fpn = addr->oldestdirty; !done; fpn = addr->oldestdirty, pagesfreed++,
        pagestofree--, pagesmustfree = pagesmustfree ? pagesmustfree - 1 : 0)
    {
        size_t freepagepfidx;
        if (!fpn || !pagestofree ||
            (!pagesmustfree &&
             addr->opcount - fpn->age < USERMODEPAGEALLOCATOR_FREEPAGECACHEAGE(
                                          addr->usedpages, addr->freepages)))
        {
            done = 1;
#ifdef DEBUG
/*printf("FREEING %u pages\n", pagesfreed);*/
        ...
        {/* Not actually needed, and it interferes with physical page emulation.
         OSRemapMemoryPagesOntoAddrs(memtodecommit.addrs, memtodecommitidx,
         memtodecommit.pageframes, &addr->OSreservedata);*/
        decommitpages:
            if (memtodecommit.idx)
            {
                if (memtodecommit.idx ==
                    OSReleaseMemoryPages(memtodecommit.pageframes,
                                         memtodecommit.idx,
                                         &addr->OSreservedata))
                {
                    memtodecommit.idx = 0;
                ...
                else
                {
                    ...
                    for (n = 0; n < memtodecommit.idx; n++)
                    {
                        ...
                        freepagepfidx =
                          ((size_t)fpn->freepage - (size_t)addr) / PAGE_SIZE;
...
{/* Returns 1 if address space was freed */
    int fromback;
    AddressSpaceReservation_t *RESTRICT addr =
      AddressSpaceFromMem(&fromback, mem);
    if (addr)
    {
        size_t n, sizeinpages = size / PAGE_SIZE;
        void *mempage = mem;
        ...
        {
            fprintf(stderr,
                    "User Mode Page Allocator: Attempt to free memory in dead "
                    "man's land\n");
            assert(0);
            abort();
        }
        for (n = 0; n < sizeinpages; n++, pagemappings++,
            mempage = (void *)((size_t)mempage + PAGE_SIZE))
        {
            if (pagemappings->pageframe && !ISPAGEFREE(*pagemappings))
            {
        ...
        if ((dofreesystemmemorycheck =
               !(addr->opcount++ &
                 (USERMODEPAGEALLOCATOR_SYSTEMFREEMEMORYCHECKRATE - 1))) ||
            addr->freepages > USERMODEPAGEALLOCATOR_FREEPAGECACHESIZE(
                                addr->usedpages, addr->freepages))
        {
            size_t pagestofree = USERMODEPAGEALLOCATOR_FREEPAGECACHESIZE(
                     addr->usedpages, addr->freepages),
                   pagesmustfree = 0;
            ...
            pagestofree =
              pagestofree > addr->usedpages ? 0 : addr->usedpages - pagestofree;
            pagesmustfree = (size_t)(addr->freepages * memorypressurescale);
            pagestofree += pagesmustfree;
#if 1
            /* Don't bother with the overhead of freeing memory unless you have
             * a sizeable chunk to do at once */
        ...
        if (!dontfreeVA)
        {
            if ((size_t)addr->frontptr - size == (size_t)mem ||
                (size_t)addr->backptr == (size_t)mem)
            {
                if (fromback)
                    addr->backptr = (void *)((size_t)addr->backptr + size);
                else
...
static INLINE void
SwapPagesBatchDecommit(AddressSpaceReservation_t *RESTRICT addr,
                       RemapMemoryPagesBlock *RESTRICT memtodecommit)
{
    if (memtodecommit->idx)
    {
        OSRemapMemoryPagesOntoAddrs(memtodecommit->addrs, memtodecommit->idx,
                                    NULL, &addr->OSreservedata);
        memtodecommit->idx = 0;
    }
}
static void SwapPagesBatchCommit(AddressSpaceReservation_t *RESTRICT addr,
                                 RemapMemoryPagesBlock *RESTRICT memtodecommit,
                                 RemapMemoryPagesBlock *RESTRICT memtocommit)
{
...
{
    int destfromback;
    AddressSpaceReservation_t *destaddr =
                                AddressSpaceFromMem(&destfromback, dest),
                              *srcaddr = AddressSpaceFromMem(0, start);
    size_t pages, n, freepages = 0, usedpages = 0,
                     destpfidx = ((size_t)dest - (size_t)destaddr) / PAGE_SIZE,
                     srcpfidx = ((size_t)start - (size_t)srcaddr) / PAGE_SIZE;
    ...
    destptr = dest;
    destpf = destaddr->pagemapping + destpfidx;
    srcptr = start;
    srcpf = srcaddr->pagemapping + srcpfidx;
    for (n = 0; n < pages; n++, destptr = (void *)((size_t)destptr + PAGE_SIZE),
        destpf++, srcptr = (void *)((size_t)srcptr + PAGE_SIZE), srcpf++)
    {
        PageMapping t;
        ...
        if (srcpf->pageframe)
        {
            ...
            if (REMAPMEMORYPAGESBLOCKSIZE == ++memtodecommit.idx)
                SwapPagesBatchDecommit(srcaddr, &memtodecommit);
        }
        if (destpf->pageframe)
        {
            memtodecommit.addrs[memtodecommit.idx] = destptr;
            if (REMAPMEMORYPAGESBLOCKSIZE == ++memtodecommit.idx)
                SwapPagesBatchDecommit(destaddr, &memtodecommit);
        }
...
/* Do nothing, as either it doesn't matter or pointers come out the same */
            ...
            if (srcIsFree)
            {
                assert(srcpf->freepagenode->freepage == destaddr);
            }
            if (destIsFree)
            {
                assert(destpf->freepagenode->freepage == srcaddr);
            }
#endif
        }
        else
        {
            FreePageNode *RESTRICT fpn =
              srcIsFree ? srcpf->freepagenode : destpf->freepagenode;
        ...
        if (srcpf->pageframe)
        {
            ...
            if (REMAPMEMORYPAGESBLOCKSIZE == ++memtocommit.idx)
                SwapPagesBatchCommit(srcaddr, &memtodecommit, &memtocommit);
            if (srcIsFree)
                freepages--;
            else
                usedpages--;
...
#endif
#define FREEPAGENODESPERSTORAGE \
    ((FREEPAGENODESTORAGESIZE - 4 * sizeof(void *)) / sizeof(FreePageNode))
...
static FreePageNodeStorage_t *FindFreePageNodeStorage(void)
{
    ...
    if ((ofpns->next = fpns = (FreePageNodeStorage_t *)AllocatePages(
           0, FREEPAGENODESTORAGESIZE, USERPAGE_TOPDOWN)))
    {
#ifdef DEBUG
/*printf("FreePageNodeStorage extends %p to %p\n", fpns,
 * ((size_t)fpns+FREEPAGENODESTORAGESIZE));*/
#endif
#if !MMAP_CLEARS
...
static INLINE FreePageNode *AllocateFPN(void)
{
    ...
    {
        if (!(lastfpnstorage = FindFreePageNodeStorage()))
            return 0;
    }
#ifdef DEBUG
    {
 ...
 * ((size_t)(fpns->next)+FREEPAGENODESTORAGESIZE));*/
#endif
...
static INLINE void FreeFPN(FreePageNode *node)
{
    ...
    assert(node >= fpns->freepagenodes &&
           node < fpns->freepagenodes + FREEPAGENODESPERSTORAGE);
    memset((void *)((size_t)node + 2 * sizeof(void *)), 0,
           sizeof(FreePageNode) - 2 * sizeof(void *));
    node->older = fpns->freefpns;
    fpns->freefpns = node;
    /* If I'm empty and there is another storage after me, check if we need to
     * free storage */
    /*if(FREEPAGENODESPERSTORAGE==++fpns->freeitems && fpns->next &&
      FREEPAGENODESPERSTORAGE==fpns->next->freeitems)
...
#endif
#define REGIONSPERSTORAGE \
    ((REGIONSTORAGESIZE - 4 * sizeof(void *)) / sizeof(region_node_t))
...
static RegionStorage_t *FindFreeRegionNodeStorage(void)
{
    ...
    if ((ofpns->next = fpns = (RegionStorage_t *)AllocatePages(
           0, REGIONSTORAGESIZE, USERPAGE_TOPDOWN)))
    {
#ifdef DEBUG
/*printf("RegionNodeStorage extends %p to %p\n", fpns,
 * ((size_t)fpns+REGIONSTORAGESIZE));*/
#endif
#if !MMAP_CLEARS
...
static INLINE region_node_t *AllocateRegionNode(void)
{
    ...
    {
        if (!(lastregionstorage = FindFreeRegionNodeStorage()))
            return 0;
    }
#ifdef DEBUG
    {
        ...
        for (t = &regionstorage; t; t = t->next)
        {
            ...
            assert(!t->freeitems ||
                   (t->freeregions >= t->regions &&
                    t->freeregions < t->regions + REGIONSPERSTORAGE));
    ...
    assert(ret >= lastregionstorage->regions &&
           ret < lastregionstorage->regions + REGIONSPERSTORAGE);
    ...
    if (fpns->next)
    {
        if (CheckFreeRegionNodeStorages(fpns->next))
        {
            ...
            DebugPrint("RegionNodeStorage releases %p to %p\n", fpns->next,
                       (void *)((size_t)(fpns->next) + REGIONSTORAGESIZE));
#endif
...
static INLINE void FreeRegionNode(region_node_t *node)
{
    ...
    memset((void *)((size_t)node + 2 * sizeof(void *)), 0,
           sizeof(region_node_t) - 2 * sizeof(void *));
    ...
    /* If I'm empty and there is another storage after me, check if we need to
     * free storage */
    /*if(REGIONSPERSTORAGE==fpns->freeitems && fpns->next &&
      REGIONSPERSTORAGE==fpns->next->freeitems)
...
static void userpage_validatestate(MemorySource *source)
{
    ...
    {
        AddressSpaceReservation_t *RESTRICT addr =
          AddressSpaceFromMem(&fromback, rfree->start);
        ...
        assert(rfree == r);
        /* Ensure that every free block never has another free block preceding
         * or postceding it */
        if (rfree->prev)
        {
            AddressSpaceReservation_t *RESTRICT addr2 =
              AddressSpaceFromMem(&fromback, rfree->prev->start);
            assert(addr != addr2 ||
                   !REGION_HASNODEHEADER(regionL_tree_s, rfree->prev, linkL));
        }
        if (rfree->next)
        {
            AddressSpaceReservation_t *RESTRICT addr2 =
              AddressSpaceFromMem(&fromback, rfree->next->start);
            assert(addr != addr2 ||
                   !REGION_HASNODEHEADER(regionL_tree_s, rfree->next, linkL));
        }
        /* Ensure that this free block lives in an address reservation */
        assert(addr);
        /* Ensure that any pages in this free block are definitely marked as
         * free */
        for (n = pagepfidx; n < ((size_t)rfree->end - (size_t)addr) / PAGE_SIZE;
             n++)
        {
            if (addr->pagemapping[n].pageframe)
            {
                ...
                assert(fpn->freepage == (void *)((size_t)rfree->start +
                                                 (n - pagepfidx) * PAGE_SIZE));
            }
        }
    }
    ...
    assert(!REGION_HASNODEHEADER(regionL_tree_s, source->lastregion, linkL));
        ...
        for (r = source->firstregion; r; r = r->next)
        {
            int mode;
            AddressSpaceReservation_t *RESTRICT addr =
              AddressSpaceFromMem(&fromback, r->start);
            size_t n, pagepfidx = ((size_t)r->start - (size_t)addr) / PAGE_SIZE;
            assert(r->end > r->start);
            /* Ensure that this block lives in an address reservation */
            assert(addr);
            assert((r->start >= addr->front && r->end <= addr->frontptr) ||
                   (r->start >= addr->backptr && r->end <= addr->back));
            ...
            {
                AddressSpaceReservation_t *RESTRICT addr2 =
                  AddressSpaceFromMem(&fromback, r->prev->start);
            ...
            {
                assert(fromback ? source->lastregion == r
                                : source->firstregion == r);
            }
            if (r->next)
            {
                AddressSpaceReservation_t *RESTRICT addr2 =
                  AddressSpaceFromMem(&fromback, r->next->start);
            ...
            {
                assert(fromback ? source->firstregion == r
                                : source->lastregion == r);
            }
            /* Ensure that any pages in this block are contiguously allocated */
            mode = 0;
            for (n = pagepfidx; n < ((size_t)r->end - (size_t)addr) / PAGE_SIZE;
                 n++)
            {
                if (!mode)
                ...
                {/* mode=1: All pages from now on must be free or not allocated
                    */
                    assert(!addr->pagemapping[n].pageframe ||
                           ISPAGEFREE(addr->pagemapping[n]));
                }
            }
        }
    }
...
static void AddRegionNode(MemorySource *source, region_node_t *RESTRICT newnode,
                          int fromback)
{
...
static void RemoveRegionNode(MemorySource *source, region_node_t *RESTRICT r,
                             int fromback)
{
...
static int HandleVANonContiguity(MemorySource *source,
                                 region_node_t *RESTRICT r, int fromback)
...
/* Special flags: USERPAGE_TOPDOWN causes the allocation to be made from the top
down.
...
void *userpage_malloc(size_t toallocate, unsigned flags)
{
    ...
    size_t size = mremapvalue ? ((flags & M2_RESERVE_ISMULTIPLIER)
                                   ? toallocate * mremapvalue
                                   : (size_t)1 << mremapvalue)
                              : toallocate;
    if (size < toallocate)
        size = toallocate;
    if (!addressspacereservation)
    {
        size_t topreload =
          USERMODEPAGEALLOCATOR_FREEPAGECACHEPRELOAD(OSSystemMemoryPressure()) *
          PAGE_SIZE;
        if (topreload > size)
        {
#ifdef DEBUG
    ...
    /* Firstly find out if there is a free slot of sufficient size and if so use
    that.
    ...
    {/* Reserve sufficient new address space */
        ...
        if ((ret = AllocatePages(0, size, USERPAGE_NOCOMMIT | flags)))
        {
            ...
            if (source->lastregion &&
                ((flags & USERPAGE_TOPDOWN)
                   ? source->lastregion->start != newnode->end
                   : source->lastregion->end != newnode->start))
            {
                if (!HandleVANonContiguity(source, newnode,
                                           flags & USERPAGE_TOPDOWN))
                    goto mfail;
            }
...
static void ConsolidateNextIntoRegion(MemorySource *RESTRICT source,
                                      region_node_t *RESTRICT r, int fromback)
{
...
int userpage_free(void *mem, size_t size)
{
    ...
    node.start = mem;
    node.end =
      0; /* size may actually be quite a lot smaller than region size */
#if USE_LOCKS
    ACQUIRE_LOCK(&userpagemutex);
#endif
    ...
    prevIsFree = r->prev &&
                 REGION_HASNODEHEADER(regionL_tree_s, r->prev, linkL) &&
                 r->prev->end == r->start;
    nextIsFree = r->next &&
                 REGION_HASNODEHEADER(regionL_tree_s, r->next, linkL) &&
                 r->next->start == r->end;
    if (nextIsFree)
    {/* Consolidate into r */
    ...
    {/* We did shrink VA, so remove newly freed region from allocated list */
        if (source->lastregion &&
            (fromback ? source->lastregion->end == r->start
                      : source->lastregion->start == r->end))
...
void *userpage_realloc(void *mem, size_t oldsize, size_t newsize, int flags,
                       unsigned flags2)
{
    void *ret = 0;
    region_node_t node, *RESTRICT r = 0;
    ...
    node.start = mem;
    node.end =
      (void *)((size_t)mem + newsize); /* not actually used for the search */
#if USE_LOCKS
    ACQUIRE_LOCK(&userpagemutex);
#endif
    if ((addr = AddressSpaceFromMem(&fromback, mem)))
    ...
    {/* We ordinarily have the region's size available to us, but if he wants
     more then can we expand? */
        ...
        {
            if ((size_t)addr->backptr - (size_t)addr->frontptr <
                newsize - regionsize)
                goto relocate;
            /* Extend VA and r */
            r->end = addr->frontptr =
              (void *)((size_t)addr->frontptr + newsize - regionsize);
            /* Fall through */
        }
        else
            goto relocate;
    ...
    if (newsize > oldsize)
    {/* Ensure there are pages up to newsize, but you need to be careful if
        you're extending VA */
...
relocate:
    ...
    if (!(flags & MREMAP_MAYMOVE))
        goto mfail;
    if (MFAIL ==
        (ret = userpage_malloc(newsize, flags2 | USERPAGE_NOCOMMIT |
                                          (fromback ? USERPAGE_TOPDOWN : 0))))
        goto mfail;
#ifdef DEBUG
/*printf("Need to realloc from %p-%p to %p-%p\n", mem, (void *)((size_t) mem +
 * oldsize), ret, (void *)((size_t) ret + newsize));*/
#endif
...
oldsize), (void *)((size_t) ret + newsize));
printf("Swapping pages from %p-%p to %p-%p\n", mem, (void *)((size_t) mem +
oldsize), ret, (void *)((size_t) ret + oldsize));*/
#endif
#if 0
