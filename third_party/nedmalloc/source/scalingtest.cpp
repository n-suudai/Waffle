/* scalingtest.cpp
Tests how various allocators scale according to block size using a monte-carlo
approach
...
template <void (*_free)(void *)>
void wrapfree(void *mem, size_t size)
{
    _free(mem);
}
...
static Allocator allocators[] = {
  ...
  {"nedmalloc", "nedmalloc",          0,                           0,
   true,        &nedalloc::nedmalloc, &wrapfree<nedalloc::nedfree>},
  {"dlmalloc", "dlmalloc", 0, 0, true, &dlmalloc, &dlfree},
#ifndef WIN32
  {"System mmap()", "sysmmap",     PAGE_SIZE,      0,
   true,            &mmap_wrapper, &munmap_wrapper},
  {"System mmap(MAP_POPULATE)", "sysmmappop",     PAGE_SIZE,      0,
   true,                        &mmappop_wrapper, &munmap_wrapper},
  {"System mmap(MAP_POPULATE, notraverse)", "sysmmappop_notraverse",
   PAGE_SIZE,                               0,
   false,                                   &mmappop_wrapper,
   &munmap_wrapper},
#else
  {"System VirtualAlloc()", "sysmmap",     PAGE_SIZE,      0,
   true,                    &mmap_wrapper, &munmap_wrapper},
#endif
  {"User mode page allocator", "usermodemmap",          PAGE_SIZE,            0,
   true,                       &userpagemalloc_wrapper, &userpagefree_wrapper},
  {"User mode page allocator (notraverse)", "usermodemmap_notraverse",
   PAGE_SIZE,                               0,
   true,                                    &userpagemalloc_wrapper,
   &userpagefree_wrapper},
...
int main(void)
{
    ...
    if (allocatoridx > sizeof(allocators) / sizeof(Allocator))
        return 1;
    Allocator &allocator = allocators[allocatoridx];
    allocator.minsizeshift = allocator.minsize
                               ? nedtriebitscanr(allocator.minsize)
                               : (allocator.minsize = 1 << 3, 3);
    printf("\nYou chose allocator %u (%s) with minsizeshift=%lu\n",
           allocatoridx + 1, allocator.name,
           (unsigned long)allocator.minsizeshift);
    // if(allocator.malloc==&userpagemalloc_wrapper)
    {
        // printf("Preallocating user mode page allocator memory ... \n");
        // userpage_free(userpage_malloc(3ULL*1024*1024*1024, 0),
        // 3ULL*1024*1024*1024);
    }
    ...
    for (int n = 0; n < LOOPS; n++)
    {
        ...
        {
            double randval =
              (double)rand() / (RAND_MAX / (8 * sizeof(size_t))) +
              allocator.minsizeshift;
        ...
        // if(allocator.malloc!=&userpagemalloc_wrapper)
        {
            ...
            {
                for (volatile char *p = (volatile char *)ptrp->mem,
                                   *pend = (volatile char *)ptrp->mem + blksize;
                     p < pend; p += PAGE_SIZE)
                    *p = 1;
            }
        }
    ...
    int n = 0;
    for (vector<pair<usCount, size_t>>::const_iterator it = bins.begin();
         it != bins.end(); ++it, n++)
    {
        fprintf(oh, "%u,%f,%lu\n", 1 << n, (double)it->first / it->second,
