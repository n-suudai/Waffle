...
int main(void)
{
...
#if 0
    {
      ...
      ReleasePages(region1, regionsize, 0);
        ...
        printf("Working ...\n");
#endif
        // First test: give me a large stretch of memory, and then be very mean
        // to it
        if (0)
        {
            ...
            printf("Allocating %uMb (%u pages) takes %lf ms\n",
                   regionsize / 1024 / 1024, pagesinregion,
            ...
            for (m = 1; m < ALLOCATIONS; m++)
            {
                for (i = 0; i < LOOPS; i++)
                {
                    size_t pageoffset = rand() & (pagesinregion - 1),
                           pagelength = (rand() & (pagesinregion - 1)) / 2,
                           size;
                    void *addr;
                    if (pageoffset + pagelength >= pagesinregion)
                        pagelength = pagesinregion - 1 - pageoffset;
            ...
            printf("allocate=%u, release=%u takes %lf ms (%lf ops/sec)\n",
                   allocations, releases, (end - start) / 1000000000.0,
        ...
        if(addrsize[n].addr)
          userpage_free(addrsize[n].addr, length/2);
            ...
            for (length = PAGE_SIZE; length < 4 * 1024 * 1024; length *= 2)
            {
                ...
                // printf("Length=%u: %d allocations (%uMb) takes %lf ms (%lf
                // ops/sec)\n", length, n, bytesallocated/1024/1024,
                // (end-start)/1000000000.0, n/((end-start)/1000000000000.0));
                printf("%u,%lf\n", length,
                       n / ((end - start) / 1000000000000.0));
            }
            getchar();
            for (n = 1; n < ALLOCATIONS; n++)
            {
        ...
        for (m = 1; m < ALLOCATIONS; m++)
        {
            ...
            for (i = 0; i < LOOPS; i++)
            {
                for (n = 0; n < m; n++)
                {
                    ...
                    { // Allocate
                        ...
                        if (length < PAGE_SIZE)
                            length = PAGE_SIZE;
                        if (MFAIL == (newblk = userpage_malloc(
                                        length, USERPAGE_TOPDOWN)))
                        {
...
#endif
                        newblk =
                          userpage_realloc(addrsize[n].addr, addrsize[n].size,
                                           length, MREMAP_MAYMOVE, 0);
            ...
            if (!(m & 63))
                printf("%d allocations (%uMb) takes %lf ms (%lf ops/sec)\n", m,
                       bytesallocated / 1024 / 1024, times[m] / 1000000000.0,
                       opcounts[m] / (times[m] / 1000000000000.0));
        }
    mfail:
        opcount = 0;
        start = GetUsCount();
      ...
      assert(REGION_EMPTY(&upper.regiontreeL));
        ...
        }
        {
            ...
            for (n = 1; n <= ALLOCATIONS; n++)
            {
                fprintf(oh, "%lf,%lf\n", times[n] / 1000000000.0,
                        opcounts[n] / (times[n] / 1000000000000.0));
            }
            fclose(oh);
        }
