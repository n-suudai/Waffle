...
#define _CRT_SECURE_NO_WARNINGS \
    1 /* Don't care about MSVC warnings on POSIX functions */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <assert.h>
...
#define TESTCPLUSPLUS                                                       \
    1 /* =1 to make 50% of ops have blocksize<=512. This is typical for C++ \
         allocator usage. */
#define BLOCKSIZE                                                             \
    16384 /* Test will be with blocks up to BLOCKSIZE. Try 8-16Kb for typical \
             app usage, 1Mb if you use large arrays etc. */
#define TESTTYPE 2 /* =1 for maximum speed test, =2 for randomised test */
#define TOUCH                                                               \
    0 /* Whether to touch all pages of an allocated region. Can make a huge \
...
#define THREADINIT(v, id) \
    (*v = CreateThread(NULL, 0, _threadcode, (LPVOID)(size_t)id, 0, NULL))
...
#define THREADINIT(v, id) \
    pthread_create(v, NULL, _threadcode, (void *)(size_t)id)
...
static void threadcode(int threadidx)
{
    ...
    for (n = 0; n < RECORDS * 100; n++)
    {
        ...
        else if (allocated < MAXMEMORY2 &&
                 dorealloc) /* If not TESTCPLUSPLUS, then how often realloc()
                               gets called depends on how small RECORDS is. */
        {
            allocated -= memsizes[whichmalloc](allocptr[i]);
            if (!(allocptr[i] = reallocs[whichmalloc](allocptr[i], size)))
                abort();
#if TOUCH
            {
                volatile char *mem = (volatile char *)allocptr[i];
...
#if 1
        r = myrandom(&seed);
        if (allocptr > threadstuff[threadidx].allocs &&
            (r & 65535) < 32760) /*<32760)*/
        {                        /* free */
            --toallocptr;
            --allocptr;
...
#endif
        {
            if (doRealloc && allocptr > threadstuff[threadidx].allocs &&
                (r & 1))
            {
                if (!(allocptr[-1] =
                        reallocs[whichmalloc](allocptr[-1], *toallocptr)))
                    abort();
#if TOUCH
                {
            ...
            {
                if (!(allocptr[0] = mallocs[whichmalloc](*toallocptr)))
                    abort();
#if TOUCH
                {
...
static double runtest()
{
    ...
    for (n = 0; n < THREADS; n++)
    {
        ...
        times[n] = 0;
        threadstuff[n].toalloc = toallocptr =
          calloc(RECORDS, sizeof(unsigned int));
    ...
    for (i = 0; i < 8; i++)
    {
...
#if DEBUG
        {
            ...
            for (n = 0; n < THREADS; n++)
            {
                ...
                totalops +=
                  threadstuff[n].ops.mallocs + threadstuff[n].ops.reallocs;
            }
    ...
    for (n = THREADS - 1; n >= 0; n--)
    {
        free(threadstuff[n].allocs);
        threadstuff[n].allocs = 0;
        free(threadstuff[n].toalloc);
        threadstuff[n].toalloc = 0;
    }
    return opspersec;
}
...
#pragma comment(lib, "user32.lib")
    ...
    if (0)
    {
        ...
        HeapSetInformation(win32heap, HeapCompatibilityInformation, &data,
                           sizeof(data));
        HeapQueryInformation(win32heap, HeapCompatibilityInformation, &data,
                             sizeof(data), NULL);
        if (2 != data)
        {
            printf(
              "The win32 low frag allocator won't work under a debugger!\n");
        }
        else
        {
