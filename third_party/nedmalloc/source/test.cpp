...
// Move constructors are utterly required to store aligned values in a STL
// collection
// In fact, alignment can't be guaranteed without them, so pre-C++0x you
// couldn't do this
struct
  ...
  SSEVectorType
{
  ...
  (defined(__i386__) && defined(__SSE2__))
    ...
    SSEVectorType(int a, int b, int c, int d)
    {
        data.ints.i[0] = a;
        data.ints.i[1] = b;
        data.ints.i[2] = c;
    ...
    SSEVectorType(const SSEVectorType &)
    {/* do nothing */
    }
#ifdef HAVE_CPP0XRVALUEREFS
private:
  ...
  (defined(__i386__) && defined(__SSE2__))
    ...
    void checkaddr() const
    {
        ...
        printf("SSEVectorType lives at %p and contains %u,%u,%u,%u\n", this,
               data.ints.i[0], data.ints.i[1], data.ints.i[2], data.ints.i[3]);
        if (myaddr & 15)
        {
...
struct UIntish
{
    ...
    UIntish &operator=(const UIntish &o)
    {
        value = o.value;
        return *this;
    }
...
usCount test(const char *desc)
{
    ...
    size_t capacity = vec1.capacity() + vec2.capacity(),
    ...
    printf(
      ...
      "Total time: %fms\n\n",
      desc, (time2 - time1) / 1000000000.0, (time3 - time2) / 1000000000.0,
      (time4 - time3) / 1000000000.0, (time5 - time4) / 1000000000.0,
      (time6 - time5) / 1000000000.0, 100.0 * (capacity - size) / size,
...
#pragma comment(lib, "user32.lib")
...

    {
        ...
        vector<SSEVectorType,
               nedallocator<SSEVectorType, nedpolicy::align<16>::policy>>
        SSEvector1(5);
        for (vector<SSEVectorType,
                    nedallocator<SSEVectorType, nedpolicy::align<16>::policy>>::
               const_iterator it = SSEvector1.begin();
             it != SSEvector1.end(); ++it)
            it->checkaddr();
        ...
        printf("\nInitialised to zero:\n");
        typedef vector<SSEVectorType,
                       nedallocator<SSEVectorType, nedpolicy::align<16>::policy,
                                    nedpolicy::zero<>::policy,
                                    nedpolicy::typeIsPOD<true>::policy>>
        SSEvector2Type;
        SSEvector2Type SSEvector2(5);
        for (SSEvector2Type::const_iterator it = SSEvector2.begin();
             it != SSEvector2.end(); ++it)
            it->checkaddr();
        ...
        */
        ...
        SSEVectorType *foo2 =
          New<SSEVectorType, std::allocator<SSEVectorType>>(4, 5, 6, 7);
        ...
        test<nedallocatorise<
          vector, UIntish, nedpolicy::typeIsPOD<true>::policy,
          nedpolicy::mmap<>::policy,
          nedpolicy::reserveN<26>::policy // 1<<26 = 64Mb. 10,000,000 *
                                          // sizeof(unsigned int) = 38Mb.
          >::
               value>(
