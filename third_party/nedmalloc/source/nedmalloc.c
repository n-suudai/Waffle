...
#pragma warning( \
  disable        \
  : 4232) /* address of dllimport is not static, identity not guaranteed */
...
#define fopen(f, m)                                                          \
    _fsopen((f), (m),                                                        \
            0x40 /*_SH_DENYNO*/) /* Have Windows let other programs view the \
                                    log file as it is written */
#ifndef UNICODE
...
#define NEDMALLOC_TESTLOGENTRY(tc, np, type, mspace, size, mem, alignment,
flags, returned) (((type)&ENABLE_LOGGING)&&((size)>16*1024))
#define NEDMALLOC_STACKBACKTRACEDEPTH 16*/
/*#define NEDMALLOC_FORCERESERVE(p, mem, size) (((size)>=(256*1024)) ?
 * M2_RESERVE_MULT(8) : 0)*/
...
#define MUNMAP(h, a, s)                                           \
    (!OSHavePhysicalPageSupport() ? MUNMAP_DEFAULT((h), (a), (s)) \
                                  : userpage_free((a), (s)))
#define MMAP(s, f)                                    \
    (!OSHavePhysicalPageSupport() ? MMAP_DEFAULT((s)) \
                                  : userpage_malloc((s), (f)))
#define MREMAP(addr, osz, nsz, mv)                  \
    (!OSHavePhysicalPageSupport()                   \
       ? MREMAP_DEFAULT((addr), (osz), (nsz), (mv)) \
       : userpage_realloc((addr), (osz), (nsz), (mv), 0))
#define DIRECT_MMAP(h, s, f)                \
    (!OSHavePhysicalPageSupport()           \
       ? DIRECT_MMAP_DEFAULT((h), (s), (f)) \
       : userpage_malloc((s), (f) | USERPAGE_TOPDOWN))
#define DIRECT_MREMAP(h, a, os, ns, f, f2)                      \
    (!OSHavePhysicalPageSupport()                               \
       ? DIRECT_MREMAP_DEFAULT((h), (a), (os), (ns), (f), (f2)) \
       : userpage_realloc((a), (os), (ns), (f), (f2) | USERPAGE_TOPDOWN))

/*#undef MREMAP
#define MREMAP(addr, osz, nsz, mv)         (!OSHavePhysicalPageSupport() ?
MREMAP_DEFAULT((addr), (osz), (nsz), (mv)) : MFAIL)*/
/*#undef DIRECT_MREMAP
#define DIRECT_MREMAP(h, a, os, ns, f, f2) (!OSHavePhysicalPageSupport() ?
DIRECT_MREMAP_DEFAULT((h), (a), (os), (ns), (f), (f2)) : MFAIL)*/
...
#endif
/* NEDMALLOC_FORCERESERVE is used to force malloc2 flags for normal malloc,
 * calloc et al */
...
#define NEDMALLOC_TESTLOGENTRY(tc, np, type, mspace, size, mem, alignment, \
    ...
    ((type) & ENABLE_LOGGING)
#endif
/* NEDMALLOC_STACKBACKTRACEDEPTH has the logger store a stack backtrace per
 * logged item. Slow! */
...
#endif
#else /* Probably if you're not using locks then you don't want ANY pthread \
         stuff at all */
#define TLSVAR void *
...
{
    fprintf(stderr,
            "nedmalloc: The operation %s is not supported under this build "
            "configuration\n",
            opname);
    abort();
    return 0;
...
static FORCEINLINE NEDMALLOCNOALIASATTR NEDMALLOCPTRATTR void *
CallMalloc(void *RESTRICT mspace, size_t size, size_t alignment, unsigned flags)
  THROWSPEC
{
    void *RESTRICT ret = 0;
...
#endif
#if USE_ALLOCATOR == 0
    ret = (flags & M2_ZERO_MEMORY)
            ? syscalloc(1, size)
            : sysmalloc(size); /* magic headers takes care of alignment */
#elif USE_ALLOCATOR == 1
    ret = mspace_malloc2((mstate)mspace, size, alignment, flags);
#ifdef HAVE_VALGRIND
    if (ret)
        VALGRIND_MEMPOOL_ALLOC(mspace, ret, size);
#endif
#ifndef ENABLE_FAST_HEAP_DETECTION
    if (ret)
    {
        ...
        if (!leastusedaddress ||
            (void *)((mstate)mspace)->least_addr < leastusedaddress)
            leastusedaddress = (void *)((mstate)mspace)->least_addr;
        if (!largestusedblock || truesize > largestusedblock)
            largestusedblock =
    ...
    ret = (void *)(_ret + 3);
    if (alignment)
        ret = (void *)(((size_t)ret + alignment - 1) & ~(alignment - 1));
    for (; _ret < (size_t *)ret - 2; _ret++)
        *_ret = *(size_t *)"NEDMALOC";
...
static FORCEINLINE NEDMALLOCNOALIASATTR NEDMALLOCPTRATTR void *
CallRealloc(void *RESTRICT mspace, void *RESTRICT mem, int isforeign,
            size_t oldsize, size_t newsize, size_t alignment, unsigned flags)
  THROWSPEC
{
    void *RESTRICT ret = 0;
#if USE_MAGIC_HEADERS
...
#ifdef HAVE_VALGRIND
    if (ret)
        VALGRIND_MEMPOOL_CHANGE(mspace, mem, ret, newsize);
#endif
#ifndef ENABLE_FAST_HEAP_DETECTION
    if (ret)
    {
...
static FORCEINLINE void CallFree(void *RESTRICT mspace, void *RESTRICT mem,
                                 int isforeign) THROWSPEC
{
...
#ifdef HAVE_VALGRIND
    {
        ...
        /* Mark the first two pointers in the newly freed block as used (linked
...
#ifdef WIN32
/*  On Windows for RELEASE both x86 and x64 the NT heap precedes each block with
   an eight byte header
    which looks like:
        normal: 4 bytes of size, 4 bytes of [char < 64, char < 64, char < 64 bit
   0 always set, char random ]
        mmaped: 4 bytes of size  4 bytes of [zero,      zero,      0xb,
   zero        ]

    On Windows for DEBUG both x86 and x64 the preceding four bytes is always
...
#pragma pack(push, 1)
        struct _HEAP_ENTRY
        {
            ...
            UCHAR Flags;  /* always bit 0 (HEAP_ENTRY_BUSY). bit
                            1=(HEAP_ENTRY_EXTRA_PRESENT), bit 2=normal block
                            (HEAP_ENTRY_FILL_PATTERN), bit 3=mmap block
                            (HEAP_ENTRY_VIRTUAL_ALLOC). Bit 4
                            (HEAP_ENTRY_LAST_ENTRY) could be set */
            UCHAR UnusedBytes;
            UCHAR SmallTagIndex; /* fastbin index. Always one of 0x02, 0x03,
                                    0x04 < 0x80 */
        } *RESTRICT he = ((struct _HEAP_ENTRY *)mem) - 1;
#pragma pack(pop)
        unsigned int header = ((unsigned int *)mem)[-1], mask1 = 0x8080E100,
                     result1, mask2 = 0xFFFFFF06, result2;
...
#endif
#ifdef __linux__
        /* On Linux glibc uses ptmalloc2 (really dlmalloc) just as we do, but
        prev_foot contains rubbish
        when the preceding block is allocated because ptmalloc2 finds the local
        mstate by rounding the ptr
        ...
        */
        ...
        if (!is_aligned(fm))
            return 0;
        if ((size_t)mem - (size_t)fm >= (size_t)1 << (SIZE_T_BITSIZE - 1))
            return 0;
        if (ok_magic(fm))
            return fm;
        else
        ...
        else
        {
            mchunkptr p = mem2chunk(mem);
            mstate fm = get_mstate_for(p);
            assert(ok_magic(fm)); /* If this fails, someone tried to free a
                                     block twice */
            if (ok_magic(fm))
                return fm;
        }
...
#endif
        {
            /* We try to return zero here if it isn't one of our own blocks,
            however
            the current block annotation scheme used by dlmalloc makes it
            impossible
            to be absolutely sure of avoiding a segfault.

            mchunkptr->prev_foot = mem-(2*size_t) = mstate ^ mparams.magic for
            PRECEDING block;
            mchunkptr->head      = mem-(1*size_t) = 8 multiple size of this
            block with bottom three bits = FLAG_BITS
                FLAG_BITS = bit 0 is CINUSE (currently in use unless is mmap),
            bit 1 is PINUSE (previous block currently
                            in use unless mmap), bit 2 is UNUSED and currently
            is always zero.
            */
            register void *RESTRICT leastusedaddress_ =
              leastusedaddress; /* Cache these to avoid register reloading */
            register size_t largestusedblock_ = largestusedblock;
            if (!is_aligned(mem))
                return 0; /* Would fail very rarely as all allocators return
                             aligned blocks */
            if (mem < leastusedaddress_)
                return 0; /* Simple but effective */
            {
                mchunkptr p = mem2chunk(mem);
                mstate fm = 0;
                int ismmapped = is_mmapped(p);
                if ((!ismmapped && !is_inuse(p)) || (p->head & FLAG4_BIT))
                    return 0;
                /* Reduced uncertainty by 0.5^2 = 25.0% */
                /* size should never exceed largestusedblock */
                if (chunksize(p) - overhead_for(p) > largestusedblock_)
                    return 0;
                /* Reduced uncertainty by a minimum of 0.5^3 = 12.5%, maximum
                 * 0.5^16 = 0.0015% */
                /* Having sanity checked prev_foot and head, check next block */
                if (!ismmapped &&
                    (!next_pinuse(p) || (next_chunk(p)->head & FLAG4_BIT)))
                    return 0;
...
#endif
                fm = get_mstate_for(p);
                if (!is_aligned(fm) || (void *)fm < leastusedaddress_)
                    return 0;
#if 0
...
#endif
                assert(ok_magic(
                  fm)); /* If this fails, someone tried to free a block twice */
                if (ok_magic(fm))
                    return fm;
            }
        }
...
NEDMALLOCNOALIASATTR size_t
nedblksize(int *RESTRICT isforeign, void *RESTRICT mem, unsigned flags)
  THROWSPEC
{
    if (mem)
    {
...
NEDMALLOCNOALIASATTR NEDMALLOCPTRATTR void *nedcalloc(size_t no, size_t size)
  THROWSPEC
{
    return nedpcalloc((nedpool *)0, no, size);
}
NEDMALLOCNOALIASATTR NEDMALLOCPTRATTR void *nedrealloc(void *mem, size_t size)
  THROWSPEC
{
...
NEDMALLOCNOALIASATTR NEDMALLOCPTRATTR void *nedmemalign(size_t alignment,
                                                        size_t bytes) THROWSPEC
{
    return nedpmemalign((nedpool *)0, alignment, bytes);
}
NEDMALLOCNOALIASATTR NEDMALLOCPTRATTR void *
nedmalloc2(size_t size, size_t alignment, unsigned flags) THROWSPEC
{
    return nedpmalloc2((nedpool *)0, size, alignment, flags);
}
NEDMALLOCNOALIASATTR NEDMALLOCPTRATTR void *
nedrealloc2(void *mem, size_t size, size_t alignment, unsigned flags) THROWSPEC
{
...
NEDMALLOCNOALIASATTR NEDMALLOCPTRATTR void **
nedindependent_calloc(size_t elemsno, size_t elemsize, void **chunks) THROWSPEC
{
    return nedpindependent_calloc((nedpool *)0, elemsno, elemsize, chunks);
}
NEDMALLOCNOALIASATTR NEDMALLOCPTRATTR void **
nedindependent_comalloc(size_t elems, size_t *sizes, void **chunks) THROWSPEC
{
...
static const char *LogEntryTypeStrings[] = {
  "LOGENTRY_MALLOC",           "LOGENTRY_REALLOC",
  "LOGENTRY_FREE",             "LOGENTRY_THREADCACHE_MALLOC",
  "LOGENTRY_THREADCACHE_FREE", "LOGENTRY_THREADCACHE_CLEAN",
  "LOGENTRY_POOL_MALLOC",      "LOGENTRY_POOL_REALLOC",
  "LOGENTRY_POOL_FREE",        "******************"};

struct threadcacheblk_t;
typedef struct threadcacheblk_t threadcacheblk;
struct threadcacheblk_t
{/* Keep less than 32 bytes as sizeof(threadcacheblk) is the minimum allocation
    size */
...
struct nedpool_t
{
    ...
    TLSVAR mycache; /* Thread cache for this thread. 0 for unset, negative for
                       use mspace-1 directly, otherwise is cache-1 */
...
static DWORD64 __stdcall GetModBase(HANDLE hProcess, DWORD64 dwAddr) THROWSPEC
{
    ...
    else
    {
        ...
        if (0 != VirtualQueryEx(hProcess, (LPCVOID)(size_t)dwAddr, &stMBI,
                                sizeof(stMBI)))
        {
            int n;
            ...
            MODULEINFO mi = {0};
            dwPathLen = GetModuleFileName((HMODULE)stMBI.AllocationBase, szFile,
                                          MAX_PATH);
            dwNameLen = GetModuleBaseName(
              hProcess, (HMODULE)stMBI.AllocationBase, szModuleName, MAX_PATH);
            ...
            if (!GetModuleInformation(hProcess, (HMODULE)stMBI.AllocationBase,
                                      &mi, sizeof(mi)))
            {
                // fxmessage("WARNING: GetModuleInformation() returned error
                // code %d\n", GetLastError());
            }
            if (!SymLoadModule64(hProcess, NULL,
                                 (PSTR)((dwPathLen) ? szFile : 0),
                                 (PSTR)((dwNameLen) ? szModuleName : 0),
                                 (DWORD64)mi.lpBaseOfDll, mi.SizeOfImage))
            {
                // fxmessage("WARNING: SymLoadModule64() returned error code
                // %d\n", GetLastError());
            }
            // fxmessage("%s, %p, %x, %x\n", szFile, mi.lpBaseOfDll,
            // mi.SizeOfImage, (DWORD) mi.lpBaseOfDll+mi.SizeOfImage);
            modulebase = SymGetModuleBase64(hProcess, dwAddr);
            return modulebase;
        }
    }
...
static void DoStackWalk(logentry *p) THROWSPEC
{
    ...
    if (!sym_myprocess)
    {
        ...
        DuplicateHandle(GetCurrentProcess(), GetCurrentProcess(),
                        GetCurrentProcess(), &sym_myprocess, 0, FALSE,
    ...
    if ((VOID(WINAPI *)(PCONTEXT)) - 1 == RtlCaptureContextAddr)
        RtlCaptureContextAddr = (VOID(WINAPI *)(PCONTEXT))GetProcAddress(
          GetModuleHandle(L"kernel32"), "RtlCaptureContext");
    ...
    { // This is nasty, but it works
        ...
        __except(
          ExceptionFilter(GetExceptionCode(), GetExceptionInformation(), &ct))
        {
        }
    }
    ...
    for (i2 = 0; i2 < NEDMALLOC_STACKBACKTRACEDEPTH; i2++)
    {
...
#endif
               sym_myprocess, mythread, &sf, &ct, NULL,
               SymFunctionTableAccess64, GetModBase, NULL))
            break;
        ...
        if (i) // Skip first entry relating to this function
        {
            ...
            if (SymGetModuleInfo64(sym_myprocess, sf.AddrPC.Offset, &ihm))
            {
                ...
                if (!leaf)
                    leaf = ihm.ImageName - 1;
                COPY_STRING(p->stack[i - 1].module, leaf + 1,
                            sizeof(p->stack[i - 1].module));
            }
            else
                strcpy(p->stack[i - 1].module, "<unknown>");
            // fxmessage("WARNING: SymGetModuleInfo64() returned error code
            // %d\n", GetLastError());
            ...
            if (SymGetSymFromAddr64(sym_myprocess, sf.AddrPC.Offset, &offset,
                                    ihs))
            {
                COPY_STRING(p->stack[i - 1].functname, ihs->Name,
                            sizeof(p->stack[i - 1].functname));
                if (strlen(p->stack[i - 1].functname) <
                    sizeof(p->stack[i - 1].functname) - 8)
                {
                    sprintf(strchr(p->stack[i - 1].functname, 0), " +0x%x",
                            offset);
                }
            }
            else
                strcpy(p->stack[i - 1].functname, "<unknown>");
            if (SymGetLineFromAddr64(sym_myprocess, sf.AddrPC.Offset,
                                     &lineoffset, &ihl))
            {
                char *leaf;
                ...
                if (!leaf)
                    leaf = ihl.FileName - 1;
                COPY_STRING(p->stack[i - 1].file, leaf + 1,
                            sizeof(p->stack[i - 1].file));
            }
            else
    ...
    { // Format can be <file path>(<mangled symbol>+0x<offset>) [<pc>]
        ...
        for (idx = 0; idx < end; idx++)
        {
            ...
            else if (1 == which && ')' == strings[i2][idx])
            {
                ...
                FXString symbol(rawsymbol.length()
                                  ? fxdemanglesymbol(rawsymbol, false)
                                  : rawsymbol);
                symbol.append(functname.mid(offset));
                int len =
                  FXMIN(symbol.length(), (int)sizeof(p->stack[i2].functname));
...
#endif
#endif
static FORCEINLINE logentry *LogOperation(threadcache *tc, nedpool *np,
                                          LogEntryType type, int mspace,
                                          size_t size, void *mem,
                                          size_t alignment, unsigned flags,
                                          void *returned) THROWSPEC
{
#if ENABLE_LOGGING
    if (tc->logentries &&
        NEDMALLOC_TESTLOGENTRY(tc, np, type, mspace, size, mem, alignment,
                               flags, returned))
    {
        logentry *le;
        if (tc->logentriesptr == tc->logentriesend)
        {
            ...
            le = (logentry *)CallRealloc(
              0, tc->logentries, 0, logentrieslen, (logentrieslen * 3) / 2, 0,
              M2_ZERO_MEMORY | M2_ALWAYS_MMAP | M2_RESERVE_MULT(8));
            if (!le)
                return 0;
            ...
            logentrieslen =
              (chunksize(cp) - overhead_for(cp)) / sizeof(logentry);
...
static FORCEINLINE NEDMALLOCNOALIASATTR unsigned int size2binidx(size_t _size)
  THROWSPEC
...
static NOINLINE int InitPool(nedpool *RESTRICT p, size_t capacity,
                             int threads) THROWSPEC;
static NOINLINE void RemoveCacheEntries(nedpool *RESTRICT p,
                                        threadcache *RESTRICT tc,
                                        unsigned int age) THROWSPEC
{
#ifdef FULLSANITYCHECKS
    ...
    if (tc->freeInCache)
    {
        ...
        for (n = 0; n <= THREADCACHEMAXBINS; n++, tcbptr += 2)
        {
            threadcacheblk *RESTRICT *RESTRICT tcb =
              tcbptr + 1; /* come from oldest end of list */
            /*tcsanitycheck(tcbptr);*/
            for (; *tcb && tc->frees - (*tcb)->lastUsed >= age;)
            {
                ...
                *tcb = (*tcb)->prev;
                ...
                /*tcsanitycheck(tcbptr);*/
                LogOperation(tc, p, LOGENTRY_THREADCACHE_CLEAN, age, blksize, f,
                             0, 0, 0);
            }
        }
    }
...
size_t nedflushlogs(nedpool *p, char *filepath) THROWSPEC
{
    ...
    if (p->caches)
    {
        ...
        if (haslogentries)
        {
            ...
            {
                char *bptr;
                if ((oh = fopen(filepath, "w")))
                    break;
                if (ENOSPC == errno)
                    break;
                bptr = strrchr(filepath, '.');
                if (bptr - filepath >= MAX_PATH - 6)
                    abort();
                memcpy(bptr, "!.csv", 6);
            }
            if (oh)
            {
                ...
                if (pos1 == pos2)
                    fprintf(oh,
                            "Timestamp, Pool, Operation, MSpace, Size, Block, "
                            "Alignment, Flags, Returned,\"Stack Backtrace\"\n");
                for (n = 0; n < THREADCACHEMAXCACHES; n++)
                {
                    if ((tc = p->caches[n]) && tc->logentries)
                    {
                        ...
                        {
                            const char *LogEntryTypeString = LogEntryTypeStrings
                              [size2binidx(((size_t)le->type) << 4)];
                            char stackbacktrace[16384] = "?";
#if NEDMALLOC_STACKBACKTRACEDEPTH
                            char *sbtp = stackbacktrace;
                            int i;
                            for (i = 0; i < NEDMALLOC_STACKBACKTRACEDEPTH &&
                                          le->stack[i].pc;
                                 i++)
                            {
                                sbtp += sprintf(
                                  sbtp, "0x%p:%s:%s (%s:%u),", le->stack[i].pc,
                                  le->stack[i].module, le->stack[i].functname,
                                  le->stack[i].file, le->stack[i].lineno);
                                if (sbtp >=
                                    stackbacktrace + sizeof(stackbacktrace))
                                    abort();
                            }
...
#endif
                            fprintf(oh,
                                    "%llu, 0x%p, %s, %d, %Iu, 0x%p, %Iu, 0x%x, "
                                    "0x%p,\"%s\"\n",
                                    le->timestamp, le->np, LogEntryTypeString,
                                    le->mspace, le->size, le->mem,
                                    le->alignment, le->flags, le->returned,
                                    stackbacktrace);
                        }
                        CallFree(0, tc->logentries, 0);
                        tc->logentries = tc->logentriesptr = tc->logentriesend =
                          0;
                    }
                }
                fclose(oh);
...
static NOINLINE threadcache *AllocCache(nedpool *RESTRICT p) THROWSPEC
{
...
#if ENABLE_LOGGING
    {
        ...
        tc->logentries = tc->logentriesptr = (logentry *)CallMalloc(
          p->m[0], logentrieslen * sizeof(logentry), 0,
          M2_ZERO_MEMORY | M2_ALWAYS_MMAP | M2_RESERVE_MULT(8));
        if (!tc->logentries)
        {
#if USE_LOCKS
...
static void *threadcache_malloc(nedpool *RESTRICT p, threadcache *RESTRICT tc,
                                size_t *RESTRICT _size) THROWSPEC
{
    void *RESTRICT ret = 0;
    ...
    if (blk)
    {
        ...
        *binsptr = blk->next;
        ...
        assert(nedblksize(0, blk, 0) >= sizeof(threadcacheblk) &&
               nedblksize(0, blk, 0) <= THREADCACHEMAX + CHUNK_OVERHEAD);
    ...
    {
        printf(
          "*** threadcache=%u, mallocs=%u (%f), free=%u (%f), freeInCache=%u\n",
          (unsigned int)tc->threadid, tc->mallocs,
          (float)tc->successes / tc->mallocs, tc->frees,
          (float)tc->successes / tc->frees, (unsigned int)tc->freeInCache);
    }
#endif
#ifdef FULLSANITYCHECKS
    tcfullsanitycheck(tc);
...
static NOINLINE void ReleaseFreeInCache(nedpool *RESTRICT p,
                                        threadcache *RESTRICT tc, int mymspace)
  THROWSPEC
{
...
static void threadcache_free(nedpool *RESTRICT p, threadcache *RESTRICT tc,
                             int mymspace, void *RESTRICT mem, size_t size,
                             int isforeign) THROWSPEC
{
    unsigned int bestsize;
    unsigned int idx = size2binidx(size);
    threadcacheblk *RESTRICT *RESTRICT binsptr,
      *RESTRICT tck = (threadcacheblk * RESTRICT)mem;
    assert(size >= sizeof(threadcacheblk) &&
           size <= THREADCACHEMAX + CHUNK_OVERHEAD);
#ifdef DEBUG
...
#endif
    if (bestsize !=
        size) /* dlmalloc can round up, so we round down to preserve indexing */
        size = bestsize;
    binsptr = &tc->bins[idx * 2];
    assert(idx <= THREADCACHEMAXBINS);
    if (tck == *binsptr)
    {
        fprintf(stderr,
                "nedmalloc: Attempt to free already freed memory block %p - "
                "aborting!\n",
                tck);
        abort();
    }
...
static NOINLINE int InitPool(nedpool *RESTRICT p, size_t capacity, int threads)
  THROWSPEC
{/* threads is -1 for system pool */
...
#elif USE_ALLOCATOR == 1
    if (!(p->m[0] = (mstate)create_mspace(capacity, 1)))
        goto err;
#ifdef HAVE_VALGRIND
    ...
    p->m[0]->extp = p;
#endif
    p->threads = (threads > MAXTHREADSINPOOL)
                   ? MAXTHREADSINPOOL
                   : (threads <= 0) ? DEFAULTMAXTHREADSINPOOL : threads;
done:
...
static NOINLINE mstate FindMSpace(nedpool *RESTRICT p, threadcache *RESTRICT tc,
                                  int *RESTRICT lastUsed, size_t size) THROWSPEC
{/* Gets called when thread's last used mspace is in use. The strategy
 ...
 exceed p->threads */
    ...
    if (end < p->threads)
    {
        ...
        {
            volatile struct malloc_state **_m =
              (volatile struct malloc_state **)&p->m[end];
            *_m = (p->m[end] = temp);
        }
...
NEDMALLOCNOALIASATTR NEDMALLOCPTRATTR nedpool *nedcreatepool(size_t capacity,
                                                             int threads)
  THROWSPEC
{
    nedpool *ret = 0;
    if (!poollist)
    {
        ...
        if (!(newpoollist = (PoolList *)nedpcalloc(0, 1, sizeof(PoolList) +
                                                           sizeof(nedpool *))))
            return 0;
#if USE_LOCKS
    ...
    if (poollist->length == poollist->size)
    {
        ...
        if (!(newpoollist = (PoolList *)nedprealloc(0, poollist, newsize)))
            goto badexit;
        poollist = newpoollist;
        toclearsize =
          newsize - sizeof(PoolList) - (poollist->size) * sizeof(nedpool *);
    ...
    if (!(ret = (nedpool *)nedpcalloc(0, 1, sizeof(nedpool))))
        goto badexit;
...
void neddestroypool(nedpool *p) THROWSPEC
{
    ...
    memmove(&poollist->list[n], &poollist->list[n + 1],
            (size_t) & poollist->list[poollist->length] - (size_t) &
              poollist->list[n]);
    if (!--poollist->length)
    {
...
void neddestroysyspool() THROWSPEC
{
    ...
    for (n = 0; n < THREADCACHEMAXCACHES; n++)
        p->caches[n] = (threadcache *)(size_t)(
          sizeof(size_t) > 4 ? 0xdeadbeefdeadbeefULL : 0xdeadbeefUL);
    for (n = 0; n < MAXTHREADSINPOOL + 1; n++)
        p->m[n] = (mstate)(size_t)(sizeof(size_t) > 4 ? 0xdeadbeefdeadbeefULL
                                                      : 0xdeadbeefUL);
    if (TLSFREE(p->mycache))
        abort();
#if USE_LOCKS
...
nedpool **nedpoollist() THROWSPEC
{
    ...
    if (poollist)
    {
        ...
        ACQUIRE_LOCK(&poollistlock);
#endif
        if (!(ret = (nedpool **)nedmalloc((poollist->length + 1) *
                                          sizeof(nedpool *))))
            goto badexit;
...
void nedtrimthreadcache(nedpool *p, int disable) THROWSPEC
{
    ...
    if (!mycache)
    {/* Set to mspace 0 */
        if (disable && TLSSET(p->mycache, (void *)(size_t) - 1))
            abort();
    }
    else if (mycache > 0)
...
#if defined(DEBUG)
        printf(
          "Threadcache utilisation: %lf%% in cache with %lf%% lost to other "
          "threads\n",
          100.0 * tc->successes / tc->mallocs,
          100.0 * ((double)tc->mallocs - tc->frees) / tc->mallocs);
#endif
        if (disable && TLSSET(p->mycache, (void *)(size_t)(-tc->mymspace)))
            abort();
        tc->frees++;
        RemoveCacheEntries(p, tc, 0);
        assert(!tc->freeInCache);
...
static FORCEINLINE mstate GetMSpace(nedpool *RESTRICT p,
                                    threadcache *RESTRICT tc, int mymspace,
...
static NOINLINE void GetThreadCache_cold1(nedpool *RESTRICT *RESTRICT p)
  THROWSPEC
{
    *p = &syspool;
    if (!syspool.threads)
...
static NOINLINE void GetThreadCache_cold2(nedpool *RESTRICT *RESTRICT p,
                                          threadcache *RESTRICT *RESTRICT tc,
                                          int *RESTRICT mymspace, int mycache)
  THROWSPEC
{
    if (!mycache)
    {/* Need to allocate a new cache */
        *tc = AllocCache(*p);
        if (!*tc)
        {/* Disable */
            if (TLSSET((*p)->mycache, (void *)(size_t) - 1))
                abort();
            *mymspace = 0;
        }
        else
...
static FORCEINLINE void GetThreadCache(nedpool *RESTRICT *RESTRICT p,
                                       threadcache *RESTRICT *RESTRICT tc,
                                       ...
                                       size_t *RESTRICT size) THROWSPEC
{
    ...
    {/* Already have a cache */
        ...
        *mymspace = (*tc)->mymspace;
        ...
        GetThreadCache_cold2(p, tc, mymspace, mycache);
    assert(*mymspace >= 0);
#if USE_LOCKS
    ...
    if (*tc)
    {
        if (*(unsigned int *)"NEDMALC1" != (*tc)->magic1 ||
            *(unsigned int *)"NEDMALC2" != (*tc)->magic2)
        {
            abort();
        }
    }
...
NEDMALLOCNOALIASATTR NEDMALLOCPTRATTR void *
nedpmalloc2(nedpool *p, size_t size, size_t alignment, unsigned flags) THROWSPEC
{
    void *ret = 0;
    threadcache *tc;
    int mymspace;
...
#if THREADCACHEMAX
    if (alignment <= MALLOC_ALIGNMENT && !(flags & NM_FLAGS_MASK) && tc &&
        size <= THREADCACHEMAX)
    {/* Use the thread cache */
        if ((ret = threadcache_malloc(p, tc, &size)))
        {
            if ((flags & M2_ZERO_MEMORY))
                memset(ret, 0, size);
            LogOperation(tc, p, LOGENTRY_THREADCACHE_MALLOC, mymspace, size, 0,
                         alignment, flags, ret);
        }
    }
#endif
    ...
    {/* Use this thread's mspace */
        ...
        if (ret)
            LogOperation(tc, p, LOGENTRY_POOL_MALLOC, mymspace, size, 0,
                         alignment, flags, ret);
    }
    LogOperation(tc, p, LOGENTRY_MALLOC, mymspace, size, 0, alignment, flags,
                 ret);
    return ret;
}
NEDMALLOCNOALIASATTR NEDMALLOCPTRATTR void *
nedprealloc2(nedpool *p, void *mem, size_t size, size_t alignment,
             unsigned flags) THROWSPEC
{
    ...
    {
        fprintf(stderr,
                "nedmalloc: nedprealloc() called with a block not created by "
                "nedmalloc!\n");
        abort();
    }
    ...
    {/* Use the thread cache */
        if ((ret = threadcache_malloc(p, tc, &size)))
        {
            ...
            LogOperation(tc, p, LOGENTRY_THREADCACHE_MALLOC, mymspace, size,
                         mem, alignment, flags, ret);
            if (!isforeign && memsize >= sizeof(threadcacheblk) &&
                memsize <= (THREADCACHEMAX + CHUNK_OVERHEAD))
            {
                threadcache_free(p, tc, mymspace, mem, memsize, isforeign);
                LogOperation(tc, p, LOGENTRY_THREADCACHE_FREE, mymspace,
                             memsize, mem, 0, 0, 0);
            }
            else
            {
                CallFree(0, mem, isforeign);
                LogOperation(tc, p, LOGENTRY_POOL_FREE, mymspace, memsize, mem,
                             0, 0, 0);
            }
        }
    }
    ...
    {/* Reallocs always happen in the mspace they happened in, so skip
     locking the preferred mspace for this thread */
        ret = CallRealloc(p->m[mymspace], mem, isforeign, memsize, size,
                          alignment, flags);
        if (ret)
            LogOperation(tc, p, LOGENTRY_POOL_REALLOC, mymspace, size, mem,
                         alignment, flags, ret);
    }
    LogOperation(tc, p, LOGENTRY_REALLOC, mymspace, size, mem, alignment, flags,
                 ret);
    return ret;
}
NEDMALLOCNOALIASATTR void nedpfree2(nedpool *p, void *mem, unsigned flags)
  THROWSPEC
{/* Frees always happen in the mspace they happened in, so skip
 locking the preferred mspace for this thread */
...
#ifdef DEBUG
        fprintf(stderr,
                "nedmalloc: WARNING nedpfree() called with zero. This is not "
                "portable behaviour!\n");
#endif
        return;
    }
    ...
    {
        fprintf(stderr,
                "nedmalloc: nedpfree() called with a block not created by "
                "nedmalloc!\n");
        abort();
    }
    GetThreadCache(&p, &tc, &mymspace, 0);
#if THREADCACHEMAX
    if (mem && tc && !isforeign && memsize >= sizeof(threadcacheblk) &&
        memsize <= (THREADCACHEMAX + CHUNK_OVERHEAD))
    {
        threadcache_free(p, tc, mymspace, mem, memsize, isforeign);
        LogOperation(tc, p, LOGENTRY_THREADCACHE_FREE, mymspace, memsize, mem,
                     0, 0, 0);
    }
    else
#endif
    {
        CallFree(0, mem, isforeign);
        LogOperation(tc, p, LOGENTRY_POOL_FREE, mymspace, memsize, mem, 0, 0,
                     0);
    }
    LogOperation(tc, p, LOGENTRY_FREE, mymspace, memsize, mem, 0, 0, 0);
}
NEDMALLOCNOALIASATTR NEDMALLOCPTRATTR void *nedpmalloc(nedpool *p, size_t size)
  THROWSPEC
{
    unsigned flags = NEDMALLOC_FORCERESERVE(p, 0, size);
    return nedpmalloc2(p, size, 0, flags);
}
NEDMALLOCNOALIASATTR NEDMALLOCPTRATTR void *nedpcalloc(nedpool *p, size_t no,
                                                       size_t size) THROWSPEC
{
...
NEDMALLOCNOALIASATTR NEDMALLOCPTRATTR void *nedprealloc(nedpool *p, void *mem,
                                                        size_t size) THROWSPEC
{
...
#endif
    {/* If he reallocs even once, it's probably wise to turn on address space
     reservation.
     If the size is larger than mmap_threshold then it'll set the reserve. */
        if (!(flags & M2_RESERVE_MASK))
            flags = M2_RESERVE_MULT(8);
    }
    return nedprealloc2(p, mem, size, 0, flags);
}
NEDMALLOCNOALIASATTR NEDMALLOCPTRATTR void *
nedpmemalign(nedpool *p, size_t alignment, size_t bytes) THROWSPEC
{
    unsigned flags = NEDMALLOC_FORCERESERVE(p, 0, bytes);
...
NEDMALLOCNOALIASATTR void *nedmalloc_internals(size_t *granularity,
                                               size_t *magic) THROWSPEC
{
#if USE_ALLOCATOR == 1
    if (granularity)
        *granularity = mparams.granularity;
    if (magic)
        *magic = mparams.magic;
    return (void *)&syspool;
#else
...
static FORCEINLINE NEDMALLOCNOALIASATTR NEDMALLOCPTRATTR void **
CallIndependentCalloc(void *RESTRICT m, size_t elemsno, size_t elemsize,
                      void **chunks) THROWSPEC
{
    ...
    {
        if (!leastusedaddress ||
            (void *)((mstate)m)->least_addr < leastusedaddress)
            leastusedaddress = (void *)((mstate)m)->least_addr;
        /*if(!largestusedblock || truesize>largestusedblock)
         * largestusedblock=(truesize+mparams.page_size) &
         * ~(mparams.page_size-1);*/
    }
    return ret;
}
NEDMALLOCNOALIASATTR NEDMALLOCPTRATTR void **
nedpindependent_calloc(nedpool *p, size_t elemsno, size_t elemsize,
                       void **chunks) THROWSPEC
{
    void **ret;
    threadcache *tc;
    ...
    if (ret && (ENABLE_LOGGING & LOGENTRY_POOL_MALLOC))
    {
        ...
        for (n = 0; n < elemsno; n++)
        {
            LogOperation(tc, p, LOGENTRY_POOL_MALLOC, mymspace, elemsize, 0, 0,
                         M2_ZERO_MEMORY, ret[n]);
        }
    }
#endif
    return ret;
...
static FORCEINLINE NEDMALLOCNOALIASATTR NEDMALLOCPTRATTR void **
CallIndependentComalloc(void *RESTRICT m, size_t elems, size_t *RESTRICT sizes,
                        void **chunks) THROWSPEC
{
    void **ret;
#if USE_ALLOCATOR == 0
...
NEDMALLOCNOALIASATTR NEDMALLOCPTRATTR void **
nedpindependent_comalloc(nedpool *p, size_t elems, size_t *sizes, void **chunks)
  THROWSPEC
{
    void **ret;
    threadcache *tc;
    ...
    for (i = 0; i < elems; i++)
        adjustedsizes[i] =
          sizes[i] < sizeof(threadcacheblk) ? sizeof(threadcacheblk) : sizes[i];
    GetThreadCache(&p, &tc, &mymspace, 0);
    ...
    if (ret && (ENABLE_LOGGING & LOGENTRY_POOL_MALLOC))
    {
        ...
        for (n = 0; n < elems; n++)
        {
            LogOperation(tc, p, LOGENTRY_POOL_MALLOC, mymspace, sizes[n], 0, 0,
                         0, ret[n]);
        }
    }
#endif
