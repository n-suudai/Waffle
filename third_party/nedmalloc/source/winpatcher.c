...
#define nedpcalloc(a, v, s) \
    HeapAlloc(GetProcessHeap(), HEAP_ZERO_MEMORY, (v) * (s))
...
/* If you want to learn lots more about the vagueries of Win32 PE dynamic
linking,
see http://msdn.microsoft.com/en-us/magazine/cc301808.aspx. Excerpt from that:

The anchor of the imports data is the IMAGE_IMPORT_DESCRIPTOR structure. The
DataDirectory
entry for imports points to an array of these structures. There's one
IMAGE_IMPORT_DESCRIPTOR
for each imported executable. The end of the IMAGE_IMPORT_DESCRIPTOR array is
indicated by an
entry with fields all set to 0. Figure 5 shows the contents of an
IMAGE_IMPORT_DESCRIPTOR.

Each IMAGE_IMPORT_DESCRIPTOR typically points to two essentially identical
arrays. These
arrays have been called by several names, but the two most common names are the
Import Address
Table (IAT) and the Import Name Table (INT).

Both arrays have elements of type IMAGE_THUNK_DATA, which is a pointer-sized
union. Each
IMAGE_THUNK_DATA element corresponds to one imported function from the
executable. The ends of
both arrays are indicated by an IMAGE_THUNK_DATA element with a value of zero.
The
...
The IMAGE_THUNK_DATA structures within the IAT lead a dual-purpose life. In the
executable
file, they contain either the ordinal of the imported API or an RVA to an
IMAGE_IMPORT_BY_NAME
structure. The IMAGE_IMPORT_BY_NAME structure is just a WORD, followed by a
string naming the
imported API. The WORD value is a "hint" to the loader as to what the ordinal of
the imported
API might be. When the loader brings in the executable, it overwrites each IAT
entry with the
actual address of the imported function. This a key point to understand before
proceeding. I
highly recommend reading Russell Osterlund's article in this issue which
describes the steps
that the Windows loader takes.

Before the executable is loaded, is there a way you can tell if an
IMAGE_THUNK_DATA structure
contains an import ordinal, as opposed to an RVA to an IMAGE_IMPORT_BY_NAME
structure? The key
is the high bit of the IMAGE_THUNK_DATA value. If set, the bottom 31 bits (or 63
bits for a 64-bit
executable) is treated as an ordinal value. If the high bit isn't set, the
IMAGE_THUNK_ DATA value
is an RVA to the IMAGE_IMPORT_BY_NAME.

The other array, the INT, is essentially identical to the IAT. It's also an
array of
IMAGE_THUNK_DATA structures. The key difference is that the INT isn't
overwritten by the loader
when brought into memory. Why have two parallel arrays for each set of APIs
imported from a DLL?
The answer is in a concept called binding. When the binding process rewrites the
IAT in the file
(I'll describe this process later), some way of getting the original information
needs to remain.
...
typedef struct SymbolListItem_t
{
    struct Replace_t
    {
        ...
        PROC addr; /* Where the symbol has this address in the DLL (=0 for use
                      GetProcAddress()) */
    } replace;
...
#elif defined(_M_X64)
    if (0xff == addr[0] && 0x25 == addr[1])
    {/* This is a jmp qword ptr, so dword[2:6] is the offset to where to load
        the address from */
...
#endif
static void putc_(void *p, char c) THROWSPEC
{
    *(*((char **)p))++ = c;
}
...
extern void DebugPrint(const char *fmt, ...) THROWSPEC
{
    ...
    if (INVALID_HANDLE_VALUE == debugfile)
    {
        debugfile = CreateFile(
          __T("C:\\nedmalloc.log"), GENERIC_READ | GENERIC_WRITE,
...
static int ExceptionToStatus(Status *ret, unsigned int code,
...
#endif
    ret->code = -(int)code;
    _stprintf_s(ret->msg, sizeof(ret->msg) / sizeof(TCHAR),
                __T("Win32 Exception %u at %p"), code,
                ep->ExceptionRecord ? ep->ExceptionRecord->ExceptionAddress
                                    : 0);
...
PVOID MyImageDirectoryEntryToData(PVOID Base, BOOLEAN MappedAsImage,
                                  USHORT DirectoryEntry, PULONG Size)
{
    ...
    offset =
      peheader->OptionalHeader.DataDirectory[DirectoryEntry].VirtualAddress;
    if (offset)
    {
        ret = (void *)((char *)Base + offset);
        if (Size)
            *Size = peheader->OptionalHeader.DataDirectory[DirectoryEntry].Size;
    }
    return ret;
}
...
*/
static Status ModifyModuleImportTableForI(HMODULE moduleBase,
                                          const char *importModuleName,
                                          SymbolListItem *sli, int patchin)
  THROWSPEC
{
    Status ret = {SUCCESS};
    ULONG size;
    ...
    PROC replaceaddr = patchin ? sli->replace.addr : sli->with.addr,
         withaddr = patchin ? sli->with.addr : sli->replace.addr;
    int replaced = 0;

    /* Find the import table of the module loaded at hmodCaller */
    desc = (PIMAGE_IMPORT_DESCRIPTOR)MyImageDirectoryEntryToData(
      moduleBase, TRUE, IMAGE_DIRECTORY_ENTRY_IMPORT, &size);
    if (!desc)
    ...
    for (; desc->Name; desc++)
    {
        ...
        if (0 == modnamecmp)
        {
            /* Get the import address table (IAT) for the functions imported
             * from our wanted module */
            thunk = (PIMAGE_THUNK_DATA)((PBYTE)moduleBase + desc->FirstThunk);

            /* Find and replace current function address with new function
             * address */
            for (; thunk->u1.Function; thunk++)
            {
                ...
                if (found)
                {
...
#if defined(_DEBUG)
                    {
                        ...
                        GetModuleBaseNameA(GetCurrentProcess(), moduleBase,
                                           moduleBaseName,
                                           sizeof(moduleBaseName) - 1);
                        DebugPrint(
                          "Winpatcher: Replacing function pointer %p (%s:%s) "
                          "with %p (%s) at %p in module %p (%s)\n",
                          *fn, importModuleName, sli->replace.name, withaddr,
                          sli->with.name, fn, moduleBase, moduleBaseName);
                    }
#endif
                    /*if(!WriteProcessMemory(GetCurrentProcess(), fn, &withaddr,
                    sizeof(withaddr), NULL))
...
#if defined(_DEBUG)
                        DebugPrint(
                          "Winpatcher: Setting PAGE_WRITECOPY on module %p, "
                          "region %p length %u\n",
...
*/
static Status ModifyModuleImportTableFor(HMODULE moduleBase,
                                         SymbolListItem *sli, int patchin,
                                         int *usingreleaseMSVCRT,
                                         int *usingdebugMSVCRT) THROWSPEC
{
    Status ret = {SUCCESS};
    int count = 0;
    for (; sli->replace.name; sli++, count++)
    {
        if (sli->modules)
        {
            ...
            {
                if (!module->intoAddr &&
                    (HMODULE)(size_t) - 1 != module->intoAddr)
                {
                ...
                if (!(sli->replace.addr = GetProcAddress(
                        sli->replace.moduleBase, sli->replace.name)))
                    return MKSTATUS(
                      ret, SUCCESS); /* Some symbols may not always be found */
                if (!module->from)
                {
                    if (!sli->with.addr)
                        abort(); /* If we are not specifying the module it must
                                    be a local symbol */
                }
                else
                {
                    ...
                    if (!GetModuleHandleExA(
                           GET_MODULE_HANDLE_EX_FLAG_UNCHANGED_REFCOUNT,
                           module->from, &withBase))
                        return MKSTATUSWIN(ret);
                    if (!(sli->with.addr =
                            GetProcAddress(withBase, sli->with.name)))
                        return MKSTATUSWIN(ret);
                }
                if ((ret = ModifyModuleImportTableForI(moduleBase, module->into,
                                                       sli, patchin),
                     ret.code) < 0)
                    return ret;
                if (ret.code && !strncmp(module->into, "MSVCR", 5))
                {
                    if (usingdebugMSVCRT && (moduleidx & 1))
                        (*usingdebugMSVCRT)++;
                    else
                        (*usingreleaseMSVCRT)++;
                }
        ...
        else
        {
            if (!sli->replace.moduleBase)
            {
                if (!sli->replace.addr || !sli->with.addr)
                    abort(); /* If you are not specifying modules you must
                                specify symbols */
                sli->replace.addr = DeindirectAddress(sli->replace.addr);
                sli->replace.moduleBase =
                  ModuleFromAddress((void *)(size_t)sli->replace.addr);
                if (!GetModuleBaseNameA(GetCurrentProcess(),
                                        sli->replace.moduleBase,
                                        sli->replace.moduleName,
                                        sizeof(sli->replace.moduleName) - 1))
                    return MKSTATUSWIN(ret);
            }
            if ((ret = ModifyModuleImportTableForI(
                   moduleBase, sli->replace.moduleName, sli, patchin),
                 ret.code) < 0)
                return ret;
        }
    }
...
/* ARA are suffering from slow process init times due to the patcher. Let's see
what we can
...
/* Modifies all symbols in all loaded modules
     Returns: The number of entries modified, plus how many are using release
   vs. debug versions of MSVCRT
*/
NEDMALLOCEXTSPEC Status WinPatcher(SymbolListItem *symbollist, int patchin,
                                   int *usingreleaseMSVCRT,
                                   int *usingdebugMSVCRT) THROWSPEC;
Status WinPatcher(SymbolListItem *symbollist, int patchin,
                  int *usingreleaseMSVCRT, int *usingdebugMSVCRT) THROWSPEC
{
    int count = 0;
    Status ret = {SUCCESS};
    __try
    {
        ...
        {
            assert(MyModuleBase);
            skipModules[0] = ModuleFromAddress(
              (void *)(size_t)StackWalk64); /* Not DbgHelp.dll as it calls
                                               malloc during its own operation
                                               which causes recursion */
        }
        /* This is not a fast call, but sadly there is no choice */
        if (!EnumProcessModules(GetCurrentProcess(), modulelist, modulelistlen,
                                &modulelistlenneeded))
            return MKSTATUSWIN(ret);
        if (!patchin || lastModuleListLen != modulelistlenneeded ||
            memcmp(lastModuleList, modulelist, modulelistlenneeded))
        {
            for (module = modulelist;
                 module < modulelist + (modulelistlenneeded / sizeof(HMODULE));
                 module++)
            {
                PatchedModule *pm = 0;
                HMODULE *m;
                ...
                {/* Already patched */
                    ...
                    DebugPrint("Winpatcher: Module %p (%s) already patched\n",
                               pm->moduleBaseAddr, pm->moduleBaseName,
                               patchin ? "patch" : "depatch");
#endif
                    if (patchin)
                    {
                        count += SUCCESS;
...
#if defined(_DEBUG)
                    /* This is an extremely slow call, so absolutely avoid if
                     * possible */
                    GetModuleBaseNameA(GetCurrentProcess(), pm->moduleBaseAddr,
                                       pm->moduleBaseName,
                                       sizeof(pm->moduleBaseName) - 1);
#endif
...
#if defined(_DEBUG)
                DebugPrint(
                  "Winpatcher: Scanning module %p (%s) for things to %s ...\n",
                  pm->moduleBaseAddr, pm->moduleBaseName,
                  patchin ? "patch" : "depatch");
#endif
                if ((ret = ModifyModuleImportTableFor(
                       *module, symbollist, patchin, usingreleaseMSVCRT,
                       usingdebugMSVCRT),
                     ret.code) < 0)
                    return ret;
                count += ret.code;
...
#if defined(_DEBUG)
            DebugPrint(
              "Winpatcher: Loaded module list hasn't changed, so exiting\n");
#endif
        }
    }
    __except(
      ExceptionToStatus(&ret, GetExceptionCode(), GetExceptionInformation()))
    {
        return ret;
    }
    return MKSTATUS(ret, count);
...
static LPVOID WINAPI VirtualAlloc_winpatcher(LPVOID lpAddress, SIZE_T dwSize,
                                             ...
                                             DWORD flProtect)
{
    ...
    if (OSHavePhysicalPageSupport())
    {
        ...
        {
            ret = userpage_malloc(
              dwSize, (flAllocationType & MEM_COMMIT) ? 0 : USERPAGE_NOCOMMIT);
#if defined(_DEBUG)
        ...
        else if (lpAddress && (flAllocationType & (MEM_COMMIT | MEM_RESERVE)) ==
                                (MEM_COMMIT))
        {
...
static BOOL WINAPI
VirtualFree_winpatcher(LPVOID lpAddress, SIZE_T dwSize, DWORD dwFreeType)
{
    dwSize = (dwSize + 4095) & ~4095;
#if defined(_DEBUG)
    ...
    if (OSHavePhysicalPageSupport())
    {
        ...
        {
            if (-1 != userpage_release(lpAddress, dwSize))
                return 1;
        }
        else if (dwFreeType == MEM_RELEASE)
        {
            if (-1 != userpage_free(lpAddress, dwSize))
                return 1;
        }
    }
    return VirtualFree(lpAddress, dwSize, dwFreeType);
}
static SIZE_T WINAPI VirtualQuery_winpatcher(LPVOID lpAddress,
...
/* The patch table: replace the specified symbols in the specified modules with
   the
   specified replacements. Format is:

   <--            what to replace               -->  <--  in what  -->  <--
   replacements            -->
   {{ "<linker symbol>", 0, "", 0|<function addr> }, 0|<which modules>, {
   "<linker symbol>", <function addr> } },

   If you specify <which modules> then <function addr> is overwritten with
   whatever
   GetProcAddress() returns for <linker symbol>. On the hand, and usually much
   more
   usefully, leaving <which modules> at zero and writing in whatever the dynamic
   linker resolves for <function addr> lets the patcher look up which modules
   the
   dynamic linker used and to patch that instead. This helps when the
   implementing
   module is not constant, but it does require that the enclosing DLL is using
   the
   same version as everything else in the process.

   Note that not specifying modules introduces x86 or x64 dependent code and
   specific
   assumptions about how MSVC implements the PE image spec. The only fully
   portable
   method is to specify modules, plus specifying modules covers executables not
   built
...
/* NOTE: Keep these release/debug format as this is used above! */
  ...
  {"MSVCR100D.DLL", 0, 0},
  {0, 0, 0}};
static ModuleListItem kernelmodule[] = {{"KERNEL32.DLL", 0, 0}, {0, 0, 0}};
static SymbolListItem nedmallocpatchtable[] = {
#ifdef NEDMALLOC_H
  {{"malloc", 0, "", 0 /*(PROC) malloc */}, modules,
   {"nedmalloc", (PROC)nedmalloc}},
  {{"calloc", 0, "", 0 /*(PROC) calloc */}, modules,
   {"nedcalloc", (PROC)nedcalloc}},
  {{"realloc", 0, "", 0 /*(PROC) realloc*/}, modules,
   {"nedrealloc", (PROC)nedrealloc}},
  {{"free", 0, "", 0 /*(PROC) free   */}, modules, {"nedfree", (PROC)nedfree}},
  {{"_msize", 0, "", 0 /*(PROC) _msize */}, modules,
   {"nedblksize", (PROC)nedmemsize}},
#endif
...
#endif
  {{"LoadLibraryA", 0, "", 0}, kernelmodule,
   {"LoadLibraryA_winpatcher", (PROC)LoadLibraryA_winpatcher}},
  {{"LoadLibraryW", 0, "", 0}, kernelmodule,
   {"LoadLibraryW_winpatcher", (PROC)LoadLibraryW_winpatcher}},
#ifdef REPLACE_SYSTEM_ALLOCATOR
#if ENABLE_USERMODEPAGEALLOCATOR
  {{"VirtualAlloc", 0, "", 0}, kernelmodule,
   {"VirtualAlloc_winpatcher", (PROC)VirtualAlloc_winpatcher}},
  {{"VirtualFree", 0, "", 0}, kernelmodule,
   {"VirtualFree_winpatcher", (PROC)VirtualFree_winpatcher}},
  {{"VirtualQuery", 0, "", 0}, kernelmodule,
   {"VirtualQuery_winpatcher", (PROC)VirtualQuery_winpatcher}},
#endif
#endif
  {{0, 0, "", 0}, 0, {0, 0}}};
#ifdef NEDMALLOC_H
/* Thunks for nedmalloc */
static void *nedmalloc_dbg(size_t size, int type, const char *filename,
                           int lineno)
{
    return nedmalloc(size);
}
static void *nedcalloc_dbg(size_t no, size_t size, int type,
                           const char *filename, int lineno)
{
    return nedcalloc(no, size);
}
static void *nedrealloc_dbg(void *ptr, size_t size, int type,
                            const char *filename, int lineno)
{
    return nedrealloc(ptr, size);
}
static void nedfree_dbg(void *ptr, int type)
{
    nedfree(ptr);
}
static size_t nedblksize_dbg(void *ptr, int type)
{
    return nedmemsize(ptr);
}

/* Here come some fun! Windows is unusual in that various DLLs loaded by
processes may be
linked to any one of the MSVCRTs listed in the patch table above - which is
twelve different
options. Each MSVCRT runs its own separate CRT heap and is generally incapable
of handling
blocks from a MSVCRT not its own, so we need to figure out specifically which
system allocator
        ...
        { // Is this me? If so keep going up the stack until it isn't
            ...
            for (module = modules; module->into; module++)
            {
                ...
                if ((HMODULE)(size_t) - 1 == module->intoAddr)
                    continue;
                desc = (PIMAGE_IMPORT_DESCRIPTOR)MyImageDirectoryEntryToData(
                  (PVOID)ihm.BaseOfImage, TRUE, IMAGE_DIRECTORY_ENTRY_IMPORT,
                  &size);
                if (!desc)
                    continue;
                for (; desc->Name; desc++)
                {
                    ...
                    {
                        DebugPrint(
                          "Module %p (%s) was originally linked to %s\n",
...
{
    /* If there is only one MSVCRT in this process, call that directly. Else do
    a stack backtrace to
...
{
    return ((void * (*)(size_t))GetProcAddress(FindMSVCRTForCaller(),
                                               "malloc"))(size);
}
static void *syscallocX(size_t no, size_t size)
{
    return ((void * (*)(size_t, size_t))GetProcAddress(FindMSVCRTForCaller(),
                                                       "calloc"))(no, size);
}
static void *sysreallocX(void *ptr, size_t size)
{
    return ((void * (*)(void *, size_t))GetProcAddress(FindMSVCRTForCaller(),
                                                       "realloc"))(ptr, size);
}
...
{
    return (
      (size_t (*)(void *))GetProcAddress(FindMSVCRTForCaller(), "_msize"))(ptr);
}
#endif
...
#endif
    ret = WinPatcher(nedmallocpatchtable, 1, &UsingReleaseMSVCRT,
                     &UsingDebugMSVCRT);
#if defined(_DEBUG)
...
{
    Status ret = {SUCCESS};
    ExceptionToStatus(&ret, ExceptionInfo->ExceptionRecord->ExceptionCode,
                      ExceptionInfo);
...
BOOL WINAPI
_DllMainCRTStartup(HANDLE hDllHandle, DWORD dwReason, LPVOID lpreserved);
BOOL WINAPI _CRT_INIT(HANDLE hDllHandle, DWORD dwReason, LPVOID lpreserved);
//#pragma optimize("", off)
/* We split DllPreMainCRTStartup to avoid an annoying bug on the x64 compiler in
/O2
whereby it inserts a security cookie check before we've initialised support for
it, thus
provoking a failure */
static PVOID ProcessExceptionHandlerH;
static __declspec(noinline) BOOL DllPreMainCRTStartup2(
  HMODULE myModuleBase, DWORD dllcode, LPVOID *isTheDynamicLinker)
{
    BOOL ret = TRUE;
    if (!MyModuleBase)
        MyModuleBase = myModuleBase;
...
#if defined(_DEBUG)
                        DebugPrint(
                          "Winpatcher: Failed to enable SeLockMemoryPrivilege. "
                          "Large pages will not be used.\n");
#endif
                        OutputDebugStringA(
                          "Winpatcher: Failed to enable SeLockMemoryPrivilege. "
                          "Large pages will not be used.\n");
...
BOOL APIENTRY DllPreMainCRTStartup(HMODULE myModuleBase, DWORD dllcode,
                                   LPVOID *isTheDynamicLinker)
{
    if (DLL_PROCESS_ATTACH == dllcode)
