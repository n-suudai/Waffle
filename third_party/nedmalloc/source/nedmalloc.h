...
NEDMALLOC_DEBUG can be defined to cause DEBUG to be set differently for
nedmalloc
...
ENABLE_LARGE_PAGES enables support for requesting memory from the system in
large
(typically >=2Mb) pages if the host OS supports this. These occupy just a single
TLB entry and can significantly improve performance in large working set
applications.
*/

/*! \def ENABLE_FAST_HEAP_DETECTION
\brief Defines whether nedalloc takes platform specific shortcuts when detecting
foreign blocks.
...
Define to enable the usage of variadic templates which enables the use of
arbitrary
numbers of policies and other useful things. Automatically defined if
__cplusplus
...
Define to enable the usage of static assertions. Automatically defined if
__cplusplus
indicates a C++0x compiler, otherwise you'll need to set it yourself.
*/
...
Define to enable the usage of &lt;type_traits&gt;. Automatically defined if
__cplusplus
indicates a C++0x compiler, otherwise you'll need to set it yourself.
*/
...
\brief Defined to the specifier for a pointer which points to a memory block.
Like NEDMALLOCNOALIASATTR, but sadly not identical. */
...
\brief Defines whether nedalloc should use magic headers in foreign heap block
detection
...
\brief Defines whether to replace the system allocator (malloc(), free() et al)
with nedalloc's implementation.
...
\brief Defines whether nedalloc should check for blocks from the system
allocator.
...
without USE_MAGIC_HEADERS there is a very tiny chance that nedmalloc will
segfault
...
*/
/*! \def THROWSPEC
\brief Defined to throw() or noexcept(true) (as in, throws nothing) under C++,
otherwise nothing.
*/
#if defined(__cplusplus)
...
\warning This API is being completely retired in v1.10 beta 2 and replaced with
the API
...
<a href="http://www.ara.com/" target="_blank">Applied Research Associates
(USA)</a>,
a new general purpose allocator API was designed which is intended to remedy
many
of the long standing problems and inefficiencies introduced by the ISO C
allocator
API. Internally nedalloc's implementations of nedmalloc(), nedcalloc(),
nedmemalign()
and nedrealloc() call into this API:

<ul>
    <li><code>void* malloc2(size_t bytes, size_t alignment, unsigned
flags)</code></li>
...
default to zero which makes the new API identical to the old API (roll on the
introduction
...
vector arrays. Hitherto SSE/AVX vector code had to jump through all sorts of
unpleasant
...
On Windows, there are two different MREMAP implementations which are chosen
according
to whether a 32 bit or a 64 bit build is being performed. The 32 bit
implementation
is based on Win32 file mappings where it reserves the address space within the
Windows
VM system, so you can safely specify silly reservation quantities like 2Gb per
block
and not exhaust local process address space. Note however that on x86 this costs
2Kb (1Kb if PAE is off) of kernel memory per Mb reserved, and as kernel memory
has
...
on x86, a much faster implementation of using VirtualAlloc(MEM_RESERVE) to
directly
reserve the address space is used.

When using M2_RESERVE_* with realloc2(), the setting only takes effect when the
mmapped chunk has exceeded its reservation space and a new reservation space
needs
to be created.
*/
...
/*! \def M2_ZERO_MEMORY
\ingroup v2malloc
\brief Sets the contents of the allocated block (or any increase in the
allocated
block) to zero.
...
malloc2() using this flag then you may have garbage just before the newly
extended
space.
...
contiguous arrays such as std::vector<> because most C++ objects cannot be
relocated
in memory without a copy or rvalue construction (though some clever STL
implementations
specialise for Plain Old Data (POD) types, and use realloc() then and only
then).
...
In the case of realloc2(), note that setting this bit will not necessarily mmap
a chunk
...
*/
#define M2_RESERVE_MULT(n) \
    (M2_RESERVE_ISMULTIPLIER | (((n) << 8) & M2_RESERVE_MASK))
...
\ingroup v2malloc
\brief Causes nedmalloc to not inspect the block being passed to see if it
belongs
...
asked for due to rounding etc. Optionally returns 1 in isforeign if the block
came from the
system allocator - note that there is a small (>0.01%) but real chance of
segfault on non-Windows
systems when passing non-nedmalloc blocks if you don't use USE_MAGIC_HEADERS.
*/
NEDMALLOCEXTSPEC NEDMALLOCNOALIASATTR size_t
nedblksize(int *RESTRICT isforeign, void *RESTRICT mem,
           unsigned flags = 0) THROWSPEC;
#else
NEDMALLOCEXTSPEC NEDMALLOCNOALIASATTR size_t
nedblksize(int *RESTRICT isforeign, void *RESTRICT mem,
...
NEDMALLOCEXTSPEC NEDMALLOCNOALIASATTR size_t
nedmemsize(void *RESTRICT mem) THROWSPEC;
...
NEDMALLOCEXTSPEC NEDMALLOCNOALIASATTR NEDMALLOCPTRATTR void *
nedmalloc(size_t size) THROWSPEC;
/*! \brief Equivalent to nedpmalloc2((nedpool *) 0, no*size, 0, M2_ZERO_MEMORY)
 */
NEDMALLOCEXTSPEC NEDMALLOCNOALIASATTR NEDMALLOCPTRATTR void *
nedcalloc(size_t no, size_t size) THROWSPEC;
/*! \brief Equivalent to nedprealloc2((nedpool *) 0, size, mem, size, 0,
 * M2_RESERVE_MULT(8)) */
NEDMALLOCEXTSPEC NEDMALLOCNOALIASATTR NEDMALLOCPTRATTR void *
nedrealloc(void *mem, size_t size) THROWSPEC;
...
NEDMALLOCEXTSPEC NEDMALLOCNOALIASATTR NEDMALLOCPTRATTR void *
nedmemalign(size_t alignment, size_t bytes) THROWSPEC;

#if defined(__cplusplus)
/*! \ingroup v2malloc
\brief Equivalent to nedpmalloc2((nedpool *) 0, size, alignment, flags) */
NEDMALLOCEXTSPEC NEDMALLOCNOALIASATTR NEDMALLOCPTRATTR void *
nedmalloc2(size_t size, size_t alignment = 0, unsigned flags = 0) THROWSPEC;
/*! \ingroup v2malloc
\brief Equivalent to nedprealloc2((nedpool *) 0, mem, size, alignment, flags) */
NEDMALLOCEXTSPEC NEDMALLOCNOALIASATTR NEDMALLOCPTRATTR void *
nedrealloc2(void *mem, size_t size, size_t alignment = 0,
            unsigned flags = 0) THROWSPEC;
/*! \ingroup v2malloc
\brief Equivalent to nedpfree2((nedpool *) 0, mem, flags) */
NEDMALLOCEXTSPEC NEDMALLOCNOALIASATTR void nedfree2(void *mem, unsigned flags =
                                                                 0) THROWSPEC;
#else
NEDMALLOCEXTSPEC NEDMALLOCNOALIASATTR NEDMALLOCPTRATTR void *
...
NEDMALLOCEXTSPEC NEDMALLOCNOALIASATTR void nedfree2(void *mem,
                                                    unsigned flags) THROWSPEC;
#endif

/*! \brief Equivalent to nedpmallinfo((nedpool *) 0) */
NEDMALLOCEXTSPEC NEDMALLOCNOALIASATTR struct nedmallinfo
nedmallinfo(void) THROWSPEC;
/*! \brief Equivalent to nedpmallopt((nedpool *) 0, parno, value) */
NEDMALLOCEXTSPEC NEDMALLOCNOALIASATTR int nedmallopt(int parno,
                                                     int value) THROWSPEC;
/*! \brief Returns the internal allocation granularity and the magic header XOR
 * used for internal consistency checks. */
NEDMALLOCEXTSPEC NEDMALLOCNOALIASATTR void *
nedmalloc_internals(size_t *granularity, size_t *magic) THROWSPEC;
...
NEDMALLOCEXTSPEC NEDMALLOCNOALIASATTR size_t
nedmalloc_footprint(void) THROWSPEC;
/*! \brief Equivalent to nedpindependent_calloc((nedpool *) 0, elemsno,
 * elemsize, chunks) */
NEDMALLOCEXTSPEC NEDMALLOCNOALIASATTR NEDMALLOCPTRATTR void **
nedindependent_calloc(size_t elemsno, size_t elemsize, void **chunks) THROWSPEC;
/*! \brief Equivalent to nedpindependent_comalloc((nedpool *) 0, elems, sizes,
 * chunks) */
NEDMALLOCEXTSPEC NEDMALLOCNOALIASATTR NEDMALLOCPTRATTR void **
...
Capacity is how much to allocate immediately (if you know you'll be allocating a
lot
of memory very soon) which you can leave at zero. Threads specifies how many
threads
will *normally* be accessing the pool concurrently. Setting this to zero means
it
extends on demand, but be careful of this as it can rapidly consume system
resources
where bursts of concurrent threads use a pool at once.
*/
NEDMALLOCEXTSPEC NEDMALLOCNOALIASATTR NEDMALLOCPTRATTR nedpool *
nedcreatepool(size_t capacity, int threads) THROWSPEC;

/*! \brief Destroys a memory pool previously created by nedcreatepool().
*/
NEDMALLOCEXTSPEC void neddestroypool(nedpool *p) THROWSPEC;

/*! \brief Returns a zero terminated snapshot of threadpools existing at the
time of call.

Call nedfree() on the returned list when you are done. Returns zero if there is
only the
...
You can retrieve this value by passing any memory block allocated from that
pool.
*/
NEDMALLOCEXTSPEC void nedpsetvalue(nedpool *p, void *v) THROWSPEC;

/*! \brief Gets a previously set value using nedpsetvalue() or zero if memory is
unknown.
...
/*! \brief Trims the thread cache for the calling thread, returning any existing
cache
data to the central pool.

Remember to ALWAYS call with zero if you used the system pool. Setting disable
to
non-zero replicates neddisablethreadcache().
*/
NEDMALLOCEXTSPEC void nedtrimthreadcache(nedpool *p, int disable) THROWSPEC;

/*! \brief Disables the thread cache for the calling thread, returning any
existing cache
data to the central pool.
...
You can pass zero for filepath to use the compiled default, or else a
char[MAX_PATH]
...
NEDMALLOCEXTSPEC NEDMALLOCNOALIASATTR NEDMALLOCPTRATTR void *
nedpmalloc(nedpool *p, size_t size) THROWSPEC;
/*! \brief Equivalent to nedpmalloc2(p, no*size, 0, M2_ZERO_MEMORY) */
NEDMALLOCEXTSPEC NEDMALLOCNOALIASATTR NEDMALLOCPTRATTR void *
nedpcalloc(nedpool *p, size_t no, size_t size) THROWSPEC;
/*! \brief Equivalent to nedprealloc2(p, mem, size, 0, M2_RESERVE_MULT(8)) */
NEDMALLOCEXTSPEC NEDMALLOCNOALIASATTR NEDMALLOCPTRATTR void *
nedprealloc(nedpool *p, void *mem, size_t size) THROWSPEC;
/*! \brief Equivalent to nedpfree2(p, mem, 0) */
NEDMALLOCEXTSPEC NEDMALLOCNOALIASATTR void nedpfree(nedpool *p,
                                                    void *mem) THROWSPEC;
...
#if defined(__cplusplus)
/*! \ingroup v2malloc
\brief Allocates a block of memory sized \em size from pool \em p, aligned to
\em alignment and according to the flags \em flags.
*/
NEDMALLOCEXTSPEC NEDMALLOCNOALIASATTR NEDMALLOCPTRATTR void *
nedpmalloc2(nedpool *p, size_t size, size_t alignment = 0,
            unsigned flags = 0) THROWSPEC;
/*! \ingroup v2malloc
\brief Resizes the block of memory at \em mem in pool \em p to size \em size,
aligned to \em alignment and according to the flags \em flags.
*/
NEDMALLOCEXTSPEC NEDMALLOCNOALIASATTR NEDMALLOCPTRATTR void *
nedprealloc2(nedpool *p, void *mem, size_t size, size_t alignment = 0,
             unsigned flags = 0) THROWSPEC;
/*! \brief Frees the block \em mem from the pool \em p according to flags \em
 * flags. */
NEDMALLOCEXTSPEC NEDMALLOCNOALIASATTR void
nedpfree2(nedpool *p, void *mem, unsigned flags = 0) THROWSPEC;
#else
NEDMALLOCEXTSPEC NEDMALLOCNOALIASATTR NEDMALLOCPTRATTR void *
nedpmalloc2(nedpool *p, size_t size, size_t alignment,
            unsigned flags) THROWSPEC;
NEDMALLOCEXTSPEC NEDMALLOCNOALIASATTR NEDMALLOCPTRATTR void *
nedprealloc2(nedpool *p, void *mem, size_t size, size_t alignment,
             unsigned flags) THROWSPEC;
NEDMALLOCEXTSPEC NEDMALLOCNOALIASATTR void nedpfree2(nedpool *p, void *mem,
                                                     unsigned flags) THROWSPEC;
#endif
...
/*! \brief Tries to release as much free memory back to the system as possible,
 * leaving \em pad remaining per threadpool. */
...
/*! \brief Returns a series of guaranteed consecutive cleared memory
  allocations.
...
*/
NEDMALLOCEXTSPEC NEDMALLOCNOALIASATTR NEDMALLOCPTRATTR void **
nedpindependent_comalloc(nedpool *p, size_t elems, size_t *sizes,
                         void **chunks) THROWSPEC;
...
Thanks to the generous support of Applied Research Associates (USA), nedalloc
has extensive
C++ language support which uses C++ metaprogramming techniques to provide a
policy driven
STL container reimplementor. The metaprogramming silently overrides or replaces
the STL implementation
on your system (MSVC and GCC are the two currently supported) to \b
substantially improve
the performance of STL containers by making use of nedalloc's additional
features.
...
The metaprogramming requires a new C++ compiler (> year 2008), and it will
readily make use
of a C++0x compiler where it will use rvalue referencing, variadic templates,
type traits and more.
Visual Studio 2008 or later is sufficent, as is GCC v4.4 or later.

nedalloc's metaprogramming is designed to be extensible, so the rest of this
page is intended for those
wishing to customise the metaprogramming. If you simply wish to know how to use
the
nedalloc::nedallocator STL allocator or the nedalloc::nedallocatorise STL
reimplementor, please refer
...
namespace nedpolicy {
    template<size_t size, size_t alignment> struct sizedalign
    {
        ...
        {
            template<class implementation> friend class
...
\endcode
The policy above implements a size based alignment, so if the block being
allocated is
less than \em size then it causes \em alignment to be used, otherwise it does
not align.
The sizedalign struct is merely a template parameter encapsulator used to
capture
additional parameters, so the real policy is in fact the class policy held
within in.
If you did not need to specify any additional parameters e.g. if you were
defining
policy_nedpool(), then you would directly define a policy returning your nedpool
and pass
it directly to nedallocator<>.

The primary policy functions which are intended to be overridden are listed in
nedalloc::nedallocatorI::baseimplementation in nedmalloc.h and are prefixed by
"policy_".
However, there is absolutely no reason why the meatier functions such as
nedalloc::nedallocatorI::baseimplementation::allocate() cannot be overriden, and
indeed
...
nedalloc::nedallocatorI::policycompositor. If you have \em really specialised
needs, you
can partially specialise this class to make it do all sorts of interesting
things - hence
...
Make SURE your message contains no spaces or anything else which would make it
an invalid
variable name.
*/
...
template <class Impl, template <class> class... policies>
class policycompositor;
template <class Impl, template <class> class A,
          template <class> class... policies>
class policycompositor<Impl, A, policies...>
{
...
#else
template <class Impl, template <class> class A = nedpolicy::empty,
...
class policycompositor
{
    typedef policycompositor<Impl, B, C, D, E, F, G, H, I, J, K, L, M, N, O>
    temp;
...
template <typename T,
          ...
          template <class> class... policies
#else
          template <class> class policy1, template <class> class policy2,
          template <class> class policy3, template <class> class policy4,
          template <class> class policy5, template <class> class policy6,
          template <class> class policy7, template <class> class policy8,
          template <class> class policy9, template <class> class policy10,
          template <class> class policy11, template <class> class policy12,
          template <class> class policy13, template <class> class policy14,
...
class baseimplementation<nedallocator<T,
                                      ...
                                      policy9, policy10, policy11, policy12,
                                      policy13, policy14, policy15
#endif
                                      >>
{
protected:
    ...
    typedef nedallocator<T,
                         ...
                         policy7, policy8, policy9, policy10, policy11,
                         policy12, policy13, policy14, policy15
#endif
                         > implementationType;
    //! \brief Returns a this for the most derived nedallocator implementation
    // type
    implementationType *_this()
    {
        return static_cast<implementationType *>(this);
    }
    //! \brief Returns a this for the most derived nedallocator implementation
    // type
    const implementationType *_this() const
    {
    ...
    //! \brief Specifies the granularity to use. Defaults to \em bytes (no
    // granularity).
    ...
    //! \brief Specifies what to do when the allocation fails. Defaults to
    // throwing std::bad_alloc.
    void policy_throwbadalloc(size_t bytes) const
    {
        throw std::bad_alloc();
    }
    //! \brief Specifies if the type is POD. Is
    // std::is_trivially_copyable<T>::value on C++0x compilers, otherwise false.
...
public:
    ...
    bool operator!=(const baseimplementation &other) const
    {
        return !(*this == other);
    }
    bool operator==(const baseimplementation &other) const
    ...
    template <typename U>
    baseimplementation(const nedallocator<U,
#ifdef HAVE_CPP0XVARIADICTEMPLATES
                                          policies...
#else
                                          policy1, policy2, policy3, policy4,
    ...
    template <typename U>
    T *allocate(const size_t n, const U * /* hint */) const
    {
        return allocate(n);
    }
...
struct granulate
{
    ...
    {
        template <class implementation>
        friend class nedallocatorI::baseimplementation;
...
struct zero
{
    ...
    protected:
        unsigned policy_flags(size_t bytes) const
        {
            return dozero ? Base::policy_flags(bytes) | M2_ZERO_MEMORY
                          : Base::policy_flags(bytes);
        }
    };
};
/*! \class preventmove
...
struct preventmove
{
    ...
    protected:
        unsigned policy_flags(size_t bytes) const
        {
            return doprevent ? Base::policy_flags(bytes) | M2_PREVENT_MOVE
                             : Base::policy_flags(bytes);
        }
    };
};
...
struct mmap
{
    ...
    protected:
        unsigned policy_flags(size_t bytes) const
        {
            return dommap ? Base::policy_flags(bytes) | M2_ALWAYS_MMAP
                          : Base::policy_flags(bytes);
        }
    };
};
...
A type specialisation exists for badalloc<void> which is equivalent to
new(nothrow)
...
On C++0x compilers, the &lt;type_traits&gt; is_trivially_copyable<type>::value
is used by default.
...
One of the lesser known features of STL container classes is their ability to
take
...
template <typename T,
          ...
          >
class nedallocator
  : public nedallocatorI::policycompositor<
#ifdef HAVE_CPP0XVARIADICTEMPLATES
      nedallocatorI::baseimplementation<nedallocator<T, policies...>>,
      policies...
#else
      nedallocatorI::baseimplementation<
        nedallocator<T, policy1, policy2, policy3, policy4, policy5, policy6,
                     policy7, policy8, policy9, policy10, policy11, policy12,
                     policy13, policy14, policy15>>,
      policy1, policy2, policy3, policy4, policy5, policy6, policy7, policy8,
      policy9, policy10, policy11, policy12, policy13, policy14, policy15
#endif
      >::value
{
...
public:
...
#endif
    /* This templated constructor and rebind() are used by MSVC's secure
    iterator checker.
    I think it's best to not copy state even though it may break policies which
    store data. */
    template <typename U>
    nedallocator(const nedallocator<U,
                                    ...
                                    policy10, policy11, policy12, policy13,
                                    policy14, policy15
#endif
                                    > &o)
    {
...
namespace nedallocatorI
{
// Holds a static allocator instance shared by anything allocating from
// allocator
...
/*! \brief Allocates the memory for an instance of object \em T and constructs
it.
...
*/
#ifdef HAVE_CPP0XVARIADICTEMPLATES
template <typename T, class allocator = nedallocator<T>, typename... Parameters>
inline T *New(const Parameters &... parameters)
#else
template <typename T, class allocator>
inline T *New()
#endif
...
// Extremely annoying not to have default template arguments for functions
// pre-C++0x
template <typename T>
inline T *New()
{
...
#define NEDMALLOC_NEWIMPL                                                    \
    \
template<typename T, class allocator, NEDMALLOC_NEWIMPLTYPES> inline T *New( \
      NEDMALLOC_NEWIMPLPARSDEFS)                                             \
    \
{                                                                     \
        allocator &a = nedallocatorI::StaticAllocator<allocator>::get();     \
        nedallocatorI::PtrHolder<T, allocator> ret(a.allocate(sizeof(T)));   \
    ...
    \
template<typename T, NEDMALLOC_NEWIMPLTYPES> inline T *New(                  \
      NEDMALLOC_NEWIMPLPARSDEFS)                                             \
    \
{                                                                     \
...
/*! \brief Destructs an instance of object T, and releases the memory used to
 * store it.
*/
template <class allocator, typename T>
inline void Delete(const T *_obj)
{
...
{
    Delete<nedallocator<T>>(obj);
}

/*! \class nedallocatorise
\ingroup C++
\brief Reimplements a given STL container to make full and efficient usage of
nedalloc
...
an example of usage.
*/
template <template <typename, class> class stlcontainer, typename T,
#ifdef HAVE_CPP0XVARIADICTEMPLATES
          template <class> class... policies
#else
          template <class> class policy1 = nedpolicy::empty,
...
\brief Defines what alignment normally returned blocks should use. Is 16 bytes
on Mac OS X, otherwise 8 bytes. */
...
\brief Defines the threshold above which mmap() is used to perform direct
allocation.
*/
#define DEFAULT_MMAP_THRESHOLD (256 * 1024)

/*! \def MAX_RELEASE_CHECK_RATE
\brief Defines how many free() ops should occur before checking how much free
memory there is.
*/
...
Note that by default realloc() sets M2_RESERVE_MULT(8) when thunking to
realloc2(),
...
You almost certainly want to constrain what gets logged using
NEDMALLOC_TESTLOGENTRY
if you turn this on as the sheer volume of data output can make execution very
slow.
*/
